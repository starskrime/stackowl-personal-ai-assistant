...
const homeBase: RightPanelProps = { mode: "home", messages: [], scrollOff: 0, recentSessions: [] };
const sessionBase: RightPanelProps = { mode: "session", messages: [], scrollOff: 0, recentSessions: [] };
...
describe("RightPanel home mode", () => {
  ...
  it("returns exactly `rows` lines", () => {
    expect(renderRightPanel(homeBase, 60, 20).lines.length).toBe(20);
  });
});
...
describe("RightPanel session mode", () => {
  ...
  it("shows conversation lines", () => {
    const props = { ...sessionBase, messages: [{ role: "assistant" as const, content: "Hello there" }] };
    const { lines } = renderRightPanel(props, 60, 20);
