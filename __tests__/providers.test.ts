...
describe("ProviderRegistry", () => {
  ...
  describe("register", () => {
    ...
    it("warns for unknown provider without baseUrl", () => {
      registry.register({ name: "unknown-provider" });
      const providers = registry.listProviders();
      expect(providers).not.toContain("unknown-provider");
    });
    ...
    it("registers openai-compatible provider", () => {
      registry.register({ name: "openai-compatible", baseUrl: "https://custom.api.com/v1", apiKey: "test-key" });
