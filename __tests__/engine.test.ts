...
function makeConfig(
  ...
  return {
    ...
    smartRouting: {
      ...
      availableModels: [
        { name: "llama3:8b", description: "Fast, lightweight" },
        { name: "llama3:70b", description: "Balanced" },
        { name: "claude-3-5-sonnet-latest", description: "Most capable" },
      ],
      ...overrides,
    },
  };
  ...
  describe("route()", () => {
    ...
    it("returns single model when only one is available", () => {
      const config = makeConfig({
        availableModels: [{ name: "llama3:8b", description: "Fast" }],
      });
      const result = ModelRouter.route("hello", config);
