...
export class ToolPruner {
  ...
  async scanAndPrune(): Promise<void> {
    console.log(
      `[ToolPruner] 🧹 Scanning Capability Ledger for failing tools...`,
    );
    ...
    for (const [toolName, info] of Object.entries(stats)) {
      ...
      if (info.consecutiveFailures >= this.FAIL_THRESHOLD) {
        console.log(
          `[ToolPruner] ⚠️ Tool "${toolName}" has failed ${info.consecutiveFailures} times consecutively. Initiating automated repair...`,
        );
  ...
  private async attemptRepair(
    ...
    try {
      ...
      if (content.includes("ACTION: ARCHIVE")) {
        console.log(
          `[ToolPruner] 🗑️ LLM elected to archive failing tool: ${toolName}`,
        );
        ...
        if (match && match[1]) {
          ...
          console.log(
            `[ToolPruner] 🩹 LLM rewrote tool: ${toolName}. Saving...`,
          );
          ...
          console.error(
            `[ToolPruner] LLM chose REWRITE but failed to provide a valid code block for ${toolName}.`,
          );
        }
      } else {
