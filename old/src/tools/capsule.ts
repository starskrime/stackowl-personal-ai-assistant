    ...
    parameters: {
      ...
      properties: {
        ...
        triggerType: {
          type: "string",
          description:
            'For create: "date" (deliver on specific date) or "condition" (deliver when condition is met)',
        },
        triggerValue: {
          type: "string",
          description:
            "For create: ISO date string (for date trigger) or natural language condition",
        },
        capsuleId: {
          type: "string",
    ...
    try {
      switch (action) {
        case "create": {
          ...
          const triggerType =
            (args.triggerType as "date" | "condition") || "date";
          const triggerValue = args.triggerValue as string;
          if (!triggerValue)
            return "Error: triggerValue is required (date or condition).";
        ...
        case "list": {
          ...
          if (capsules.length === 0)
            return 'No time capsules. Use action "create" to seal one.';
          return (
            "**Time Capsules:**\n" +
            capsules
              .map((c) => {
