      ...
      parameters: {
        ...
        properties: {
          ...
          workflowDef: {
            type: "object",
            description:
              "Workflow definition object (required for create action) containing at minimum 'id' and 'steps'.",
          },
        },
        required: ["action"],
      },
      ...
      switch (action) {
        case "list": {
          ...
          return workflows
            .map(
              (w) =>
                `- **${w.name}** (${w.id}): ${w.description} [${w.tags.join(", ")}] — run ${w.runCount}x`,
            )
            .join("\n");
        }
        ...
        case "run": {
          ...
          for (const sr of result.stepResults) {
            lines.push(
              `  ${sr.stepId}: ${sr.status}${sr.error ? ` — ${sr.error}` : ""}`,
            );
