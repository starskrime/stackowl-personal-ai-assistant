  ...
  try {
    ...
    if (data.error) {
      ...
      appendMessage(
        "assistant",
        data.content,
        state.activeOwl.name,
        state.activeOwl.emoji,
      );
    }
  } catch (e) {
    historyEl.removeChild(loadingDiv);
...
function setActiveOwl(name) {
  ...
  document.getElementById("header-owl-type").textContent =
    `${owl.type} • Challenge: ${owl.challengeLevel}`;
...
function setupEventListeners() {
  ...
  document.querySelectorAll(".tab-btn[data-target]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      document
        .querySelectorAll(".tab-btn[data-target]")
  ...
  document
    .getElementById("btn-close-parliament")
    .addEventListener("click", () => {
      document.getElementById("parliament-modal").classList.add("hidden");
    });

  document
    .getElementById("btn-cancel-parliament")
    .addEventListener("click", () => {
      document.getElementById("parliament-modal").classList.add("hidden");
    });

  document
    .getElementById("btn-start-parliament")
    .addEventListener("click", () => {
      const topic = document.getElementById("parliament-topic").value;
      const checkboxes = document.querySelectorAll(
        "#parliament-owls input:checked",
      );
...
function openPelletModal(pellet) {
  ...
  document.getElementById("reader-date").textContent = new Date(
    pellet.generatedAt,
  ).toLocaleString();
  document.getElementById("reader-owls").textContent = pellet.owls.join(", ");

  document.getElementById("reader-tags").innerHTML = pellet.tags
    .map((t) => `<span class="tag">${t}</span>`)
    .join("");
  document.getElementById("reader-body").innerHTML = marked.parse(
    pellet.content,
  );
