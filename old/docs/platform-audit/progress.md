...
| 18 | Providers (model routing, health, cost) | ✅ All 14 tasks shipped. D1: deleted `engine/router.ts` (~190 LOC, HEAVY/SIMPLE_PATTERNS regex violations) + all `smartRouting` dead code (type stub, Telegram config menu, CLI onboarding, ~57 refs). D2: `buildDefaultIntelligenceConfig()` — always construct `ctx.intelligence`, never null. D3: `ProviderCircuitBreaker` CLOSED/OPEN/HALF_OPEN state machine per provider; `getAvailable()`, `recordProviderResult()`, `isProviderOpen()` on `ProviderRegistry`. D4: `resolveCapable()` with graceful capability degrade. D5: `resolveWithCostAwareness()` with CostTracker budget accessor wired in `core.ts`. D6: `FallbackEntry`/`HealthPolicy`/`CostPolicy` types + `capabilities?` on `TierConfig`. D7: deleted `openai-compat.ts` (522 LOC dead). D8: throw on missing model config; live `listModels()` API call in anthropic-native. Q1: `PRICING_UPDATED_AT` staleness warning at startup. Net file delta: +1 created − 2 deleted = −1. 4995 tests passing. Head: `375808d`. | 2026-05-10 |
| 19 | Skills engine (match, inject, synthesize) | ✅ All 19 tasks shipped. Fixed 15 structural gaps (G1–G15): D2 wired `skillsContext` through `ContextBuilder` (G2 CRITICAL — skills XML now reaches LLM); D1 deleted `SkillsEngine` duplicate matcher (G1); Q1 removed `SKILL_ACTION_KEYWORDS` hardcoded regex (G9); D4 force-includes `always:true` skills (G4); D5 wired `invoke_skill` executor via `executeByName` (G5); D6 replaced `if(false)` with LLM confidence gate (G6); D7 deleted dead synthesis pipeline—`evolver.ts` + `pattern-miner.ts` + 3 scheduler hooks (~700 LOC, G7/G8); Q3 deleted `SkillComposer` dead abstraction (G12); Q4 fixed `withTimeout` leak with `AbortController` (G13); Q2 migrated stats from JSON to SQLite `skill_usage` table at schema v29 (G11); D8 created `SkillManagementRouter` channel-agnostic dispatcher (G10); D9 created `SkillCreationWizard` via `ChannelAdapterV2.ask()` (G10); CI-3 NL install intent detection; CI-6 `/skill metrics` verb; Q7 renamed `memoryContextPrefix`. Net file delta: +2 created, −5 deleted = −3. 5001 tests passing. Head: `eb9833e`. Final review fixes (post-`401a20d`): `skill.steps?.length` narrowed with `??0` + `!` non-null assertion; `skill.filePath` → `skill.sourcePath`; `private db` → plain `_db` param in `SkillCreationWizard` — 4 new-file TS errors eliminated, all remaining TS errors pre-existing. | 2026-05-10 |
