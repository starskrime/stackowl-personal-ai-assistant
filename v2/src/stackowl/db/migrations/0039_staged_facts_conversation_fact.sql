-- Expand staged_facts.source_type CHECK to include 'conversation_fact' so that
-- facts extracted from conversation turns by the ConversationMiner (RC-A fix)
-- are distinguishable from raw conversation turns ('conversation') and can be
-- promoted to committed_facts by the existing fact_promoter without being
-- confused with original turn data. SQLite cannot ALTER a CHECK constraint in
...
CREATE TABLE staged_facts_new (
    ...
    source_type         TEXT    NOT NULL
                            CHECK (source_type IN ('conversation', 'conversation_fact', 'parliament', 'manual', 'webpage', 'screenshot', 'agent_self')),
