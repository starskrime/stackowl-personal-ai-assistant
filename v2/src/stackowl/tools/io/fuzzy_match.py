"""Fuzzy matching substrate for file-edit operations.
...
7. Unicode normalized - Map smart quotes/dashes/ellipsis to ASCII
...
(E3 ``edit`` row). This module is a verbatim algorithm port — port-source:
upstream-agent@38441a7d7 · port-fidelity: verbatim. The only deviations from the
source are type annotations added to satisfy strict mypy (logic unchanged) and
the neutralisation of vendor names per the no-vendor-names-in-code rule.
...
def _strategy_unicode_normalized(content: str, pattern: str) -> list[tuple[int, int]]:
    ...
    making a naive position copy incorrect.
    """
