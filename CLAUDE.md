...
| 2026-09-10 | **`12886 passed, 17 skipped, 0 failed in 2560.57s` (rc=0)**, `SUITE TREE STILL` — 42nd run, on `5090a9ef`. DEBT-289 built off-tree for the whole 43 minutes. **AND I VOIDED MY OWN TARGETED RUN THE WAY THE FULL SUITE GETS VOIDED:** the first derived run came back RED on `test_pid_cleanup_handler_no_longer_raises_systemexit` — the exact canary this file already records — because I edited the mirror's `orchestrator.py` WHILE that run was executing. Re-run on the stable tree: 124 passed. **The don't-touch-the-tree rule is about any run in flight, not only the 43-minute one**, and a mirror is the cheap place to learn it. **The gate also refused the docs commit for the SECOND loop running, for the same reason: re-verifying a document EXPIRES its own `Reviewed:` dismissals** — a sha older than the new verification date is already covered. Anticipated this time and pruned before the gate ran. |
| 2026-09-10 | **`12894 passed, 17 skipped, 0 failed in 2571.63s` (rc=0)**, `SUITE TREE STILL` — 43rd run, on `b473b59b`. DEBT-290 built off-tree for the whole 43 minutes, and the mid-run edit that voided the last loop's targeted run was avoided by WAITING for it before touching the mirror again. **The item's own instrument was wrong THREE WAYS on its first run, and every one read as "the fix is missing" rather than "the test is broken"**: a class name I had not looked up, a constant whose leading underscore I invented, and `ast.parse` raising `IndentationError` on the source of a METHOD — `inspect.getsource` returns it still indented, so `textwrap.dedent` is not optional. A guard that fails for its own reasons is indistinguishable from one that found something. |
| 2026-09-10 | **`12900 passed, 17 skipped, 0 failed in 2562.57s` (rc=0)**, `SUITE TREE STILL` — 44th run, on `d778c306`. DEBT-291 built off-tree for the whole 43 minutes. **AND THE MIRROR FAILED TO MODEL THE TREE FOR A FOURTH DISTINCT REASON:** `tests/audit` reported two failures there — a documented baseline and a migration count — because both compare `docs/` (SYMLINKED to the real tree) against the MIRROR's own migrations directory and gate script, so they measure two trees against each other. Verified on the real tree: 18 passed. After `.resolve()` escaping a symlinked `scripts/`, a test that globs the repo and compares it to itself, and `write_text` following a symlinked `progress.yml`, the rule that keeps holding is the same: **check a mirror failure against the real tree before believing it.** |
| 2026-09-10 | **`12900 passed, 17 skipped, 0 failed in 2563.11s` (rc=0)**, `SUITE TREE STILL` — 45th run, on `1cf51fd8`. DEBT-292 built off-tree for the whole 43 minutes, and it is the fix DEBT-291 deliberately withheld: shipping it a loop earlier would have added a fourth unscoped accessor to a guard that could not yet see the table. **A single-principal install makes a scoped and an unscoped DELETE indistinguishable**, which is how an unscoped one survives review — so the sweep iterates `principals` rather than deleting by time, and a behaviour test (not the SQL-text tripwire) is what proves it. |
| 2026-09-10 | **`12905 passed, 17 skipped, 0 failed in 2591.60s` (rc=0)**, `SUITE TREE STILL` — 46th run, on `f2cf1a33`. DEBT-293 built off-tree for the whole 43 minutes, and **FOUR candidate seams were measured and REFUTED before it**: the 17 permanent skips (the bwrap journeys run fine here — 10 passed, 0 skipped), `undelivered_outbox` (218 rows, all 218 surfaced), the 228 functions outside the visibility gate (a recorded scope decision), and 188 goal runs that completed with an empty body — real, but ZERO since 2026-08-28. The by-day breakdown killed the last one exactly as it has four times before. **Refuting four seams IS the loop's work when the fifth is real**; the alternative is shipping the first thing that looks like a defect. |
| 2026-09-10 | **`12909 passed, 17 skipped, 0 failed in 2650.13s` (rc=0)**, `SUITE TREE STILL` — 47th run, on `29abba20`. DEBT-294 built off-tree for the whole 44 minutes, and the item was found by a NEW instrument: asking what the operator actually WAITS for. Turn latency over two days averaged 315s, and one owl — `jobmarket` — held **8.06 hours, 82% of ALL turn time**, 71% of it retries. **mypy then caught me reaching past a deliberately narrow protocol**: `_Store` lists only what the loop needs to drive a task, and widening it would force every test double to grow a budget method. The read asks through `getattr` instead. A type error can be a design review. |
| 2026-09-10 | **`12913 passed, 17 skipped, 0 failed in 2486.20s` (rc=0)**, `SUITE TREE STILL` — 48th run, on `e4c0087e`. DEBT-295 built off-tree for the whole 41 minutes. **AND THE MIRROR FAILED FOR A FIFTH DISTINCT REASON, which is the worst of them because it reaches OUT of the mirror and into the real tree.** The recipe symlinks `.venv`; `uv run` INSIDE the mirror then rewrites the SHARED venv's editable install to point at the mirror — `direct_url.json` read `file:///…/scratchpad/mirror` while a full suite was executing out of the real tree, and the two runs flipped it back and forth on every command. The tell was printed on every single invocation — `Uninstalled 2 packages / Installed 2 packages` — and it reads as noise. **Never run `uv run` or pytest inside a mirror that shares the venv: author there, execute on the real tree.** This run survived it (0 mirror paths in the log, 0 failures, and a count consistent with the real tree) because the finder's mapping is read at interpreter start, but that is luck, not isolation. |
| 2026-09-10 | **RED, AND KILLED BY ME AT 22% — no verdict**, 49th run, on `87a6e7af`. It failed at **0%**, inside the first 72 tests, and the failure was mine: `test_the_sweep_sees_a_real_population` is a VACUITY CONTROL that counts bracketed `closing_check` patterns in the LIVE record and demands three. The previous loop closed two `partial` validates — which DELETES their checks — the count fell to 2, and the run went red on a guard that had found nothing wrong. **A vacuity control whose denominator is the thing the programme exists to reduce will keep failing FOR THE PROGRAMME WORKING**, and lowering the threshold only defers it: 2 becomes 1 becomes 0. It now plants its own synthetic entry and asserts the extraction returns exactly that, with a separate live assertion that only needs to be non-empty. It is deliberately NOT a tripwire, so the gate ran clean over it four times — the full run was the only thing that could see it. **AND I KILLED THE RUN ON A WRONG BELIEF:** I read 22% and judged it crawling, when the clock said 18 minutes elapsed — a normal pace. No green verdict was lost (it was genuinely red on a cause already fixed), but the judgement was wrong and the cure is one command: **read `date -u`, never infer elapsed time from how much output you have seen.** The percentage lies about position; my memory lied about the clock. |
| 2026-09-10 | **`12943 passed, 17 skipped, 0 failed in 2510.56s` (rc=0)**, `SUITE TREE STILL` — 50th run, on `eee0cd49`, GREEN on the tree carrying DEBT-296. Launched at the START of the loop and DEBT-297 was measured, built, mutation-proven and dry-run OFF-TREE for the whole 42 minutes — against a FAKE DB rather than a mirror, which is cheaper and, for a query-shape change, stricter: the assertion that the escalation SQL carries no `timestamp >=` lives INSIDE the fake, so it cannot pass by accident. **AND THE APPLY FOUND WHAT THE FAKE STRUCTURALLY COULD NOT.** A second test file writes its own `capability.escalated` row inline with `details="{}"` — a shape no real escalation has ever had — and went red the instant the comparison went in. A fake db cannot fail a test that inserts its own row, so an off-tree proof is blind to every fixture that bypasses it. **Dry-run off-tree, then apply and run the DERIVED path before believing you are done** — the derived path is what found it. |
| 2026-09-10 | **`12950 passed, 17 skipped, 0 failed in 2507.32s` (rc=0)**, `SUITE TREE STILL` — 51st run, on `3aa65fd0`. DEBT-298 built off-tree for the whole 42 minutes. **THE ITEM CAME FROM A HANDOFF A PREVIOUS LOOP DELIBERATELY LEFT**, which is worth more than the item: DEBT-296 measured the both-DEBUG tool set, scoped it OUT with a stated reason, and printed the number so a later loop could decide with it rather than re-measure. That is the shape to repeat — a scope cut is only honest if it leaves the measurement behind. **And the rule that came out of it is DERIVED, not listed:** `ToolManifest.action_severity` already answers "does this tool change anything?", 56 tools declare it and 27 of them change something, so "a tool that changes something records its outcome at INFO+" reads 4 offenders against 23 already-correct. A rule expressed in the platform's own vocabulary needs no allowlist and cannot rot into a preference. **Its blind spot is recorded rather than papered over:** `learned_shell_tool` passes `action_severity=self._spec.action_severity` — a VARIABLE — so a MODEL-AUTHORED tool's severity is decided at runtime and no AST walk can read it. A scan over literals is blind to precisely the tools whose authority is not typed by a person. |
| 2026-09-10 | **`12953 passed, 17 skipped, 0 failed in 2589.25s` (rc=0)**, `SUITE TREE STILL` — 52nd run, on `94f3c9cf`. DEBT-299 built off-tree for the whole 43 minutes. **AND THE APPLY REFUTED A CLAIM THE RECORD ALREADY CONTAINED, which is the loop's real finding:** I had written that the new population guard stopped the walk reverting to its fail-open shape. Mutation on the applied tree said otherwise — reverting either INFO promotion failed loudly, and reverting the RULE failed **nothing**, all thirteen tests green. The two loop shapes agree on today's tree and diverge only on a tool that does not exist yet, so no assertion over the real tree can tell them apart. **A guard that pins the DATA cannot pin the RULE.** The cure is to build the tool that does not exist yet: a synthetic root of two modules, one undeclared and one declaring `read`, both logging a loud entry and a quiet exit — the correct rule reports the first, the fail-open rule reports neither. **A dry run off-tree can only prove the fix; only the mutation proves the GUARD.**
| 2026-09-11 | **`12956 passed, 17 skipped, 0 failed in 2565.43s` (rc=0)**, `SUITE TREE STILL` — 53rd run, on `df9b60bb`. DEBT-300 built off-tree for the whole 43 minutes, and the wait bought the item's best catch: **deriving the targeted path surfaced a RECORDED DECISION the sweep was one apply away from overriding.** `test_a_running_job_is_not_a_silent_store`'s docstring says "NOT A GENERAL COLUMN SWEEP … the defect here is specific to a table whose rows are created RARELY and written OFTEN", naming `message_ledger` and `delivery_attempts` as deliberate leave-alones. I had drafted a third re-dating. Measured against that criterion rather than argued — created-to-updated gap **tool_heuristics 168.00h, skills 15.27h, lessons 0.00h, message_ledger 0.01h, delivery_attempts 0.00h** — `lessons` sits with the leave-alone pair and keeps its clock. **A decision that lives only in one test's docstring is reachable only by `tests_touching.py`**, and it was the derivation, not the reading, that found it. **AND THE FIFTH MUTATION GOT AWAY:** collapsing `unknown` into `insert_only` — "cannot tell" read as "nothing to worry about" — passed all five guards, because today's unknown set is EMPTY and that branch is never taken. DEBT-299's accident one week on: a fail-open control written as an assertion about what is IN the hole cannot see an empty one. **A guard over a currently-empty population is pinned only by a population you CONSTRUCT.**
| 2026-09-11 | **`12965 passed, 17 skipped, 0 failed in 2621.26s` (rc=0)**, `SUITE TREE STILL` — 55th run, on `c8f552c3`. A01.2 built off-tree for the whole 44 minutes. **AND THE ITEM'S OWN PREMISE WAS A LITERAL I INVENTED.** I had written into SKILL.md, `progress.yml` AND a published report that effective bounds were "computed 684 times and denied zero times". Both halves were false: `bounds: DENY` is emitted by nothing — the code writes `"[pipeline] execute: tool refused by bounds"`, which has fired **96** times, 48 on 2026-09-10 alone — and the 684 counted `[authz] compose: boundary-router tools added to effective bounds`, a deliberate WIDENING. **A zero from a pattern you composed yourself is evidence of a typo, not of silence**, and one grep on the emitter settles it. The real defect underneath was better than the invented one: the ban branch builds its remainder from `all_names`, which sets `restrict_to`, which disabled the bounds narrowing twenty lines later — so a retry presents tools the owl is then refused, **59 of the 96 on retry traces**. **AND THE WAIT NARROWED THE DIFF**: reading how presentation combines `restrict_to` and `profile` showed restrict_to takes absolute precedence and DROPS profile groups, so removing both halves of the guard would have silently swapped a profiled owl's DNA groups for its bounds — unmeasurable here, since zero of eleven owls carry a profile. Only the measured half changed.
| 2026-09-11 | **`12991 passed, 17 skipped, 0 failed in 2640.22s` (rc=0)**, `SUITE TREE STILL` — 56th run, on `333b20b6`. DEBT-303 built off-tree for the whole 44 minutes, and the item was found by a PANEL READING MY OWN WORK FROM THE SAME MORNING: `receiver.py` offered "`receiver.handle` — the line a served request emits — **ZERO**" as evidence, and that message is DEBUG, so its count is zero by construction against 0 DEBUG records in 677,108. **The conclusion was RIGHT, which is what let it through** — an unfalsifiable claim that happens to be true is harder to catch than a false one. Cause: `logging_visibility.py`'s BACKGROUND is a hand-listed tuple of five and `webhooks` fits its own criterion exactly. **AND THE GENERALISED GUARD WAS BUILT, MEASURED AND REFUTED** — 2,384 DEBUG-only literals swept against every docstring, doc and record returns FOUR hits, two of them quoted source and one honest, and it could not see its own founding case because prose cites a log line by PREFIX. **THEN THE GATE REFUSED THE COMMIT AND FOUND THE SAME CAUSE IN A SECOND INSTRUMENT**: `test_a_done_validate_names_its_evidence` matched the literal prefix `validated` while the corpus writes `validate_`, so **17 of the 43 it called unevidenced named their evidence in full** — a ceiling 40% false, ratcheted against by every later loop. Matched by shape now, 43 -> 26, and eight records carrying `validate: done` beside a present-tense `validate_is_PARTIAL` key re-keyed. **Re-keying them EMPTIED the population that pinned the exclusion and the second mutation survived** — DEBT-300's lesson one week on, cured by CONSTRUCTING the population rather than counting the live one. Its old `>= 30` live floor is gone for the same reason it failed: a floor under a population this programme exists to REDUCE fails the day the work succeeds. |
| 2026-09-11 | **RED — `1 failed, 12990 passed, 17 skipped in 2656.17s` (rc=1)**, `SUITE TREE STILL`, on `1902fb7e` — 57th run, and it EARNED its place: the failure was a genuine flake the gate cannot see, on a tree shipped hours earlier. `test_foreground_write_completes_quickly_during_chunked_background_job` failed at `0.450 < 0.790/2` and passes alone every time. **THIS WAS ITS SECOND REPAIR, AND THE FIRST HAD THE RIGHT DIAGNOSIS AND THE WRONG CURE**: v1 compared a measured duration to a CONSTANT; the 2026-09-05 repair correctly observed a constant does not inflate with load and reached for `fg_elapsed < bg_total_s / 2`, still a wall-clock RATIO. **A ratio is load-proof only when numerator and denominator inflate TOGETHER, and here they do not** — one is a queue wait, the other a sum of sleeps. The property was always a COUNT: how many commits the foreground queued behind. MEASURED seven consecutive runs — **2 every time, ZERO variance**, while the two durations moved with the box (and were 0.79s vs 0.43s under full-suite load). **AND THE BETTER FINDING: both earlier versions were blind to the failure the story exists to prevent.** `pool.py:27-38` names it as a chatty loop committing one row at a time; mutation-testing that per-row loop PASSED the new count assertion AND the old duration one, because per-row autocommit releases the writer between rows — the foreground slips in fast while the job crawls, so `fg 0.09s < bg 15s / 2` is true and says nothing. **The tell was never in the assertions: the mutated run took 17.2s instead of 0.6s.** A test that takes 28x longer and still passes is not measuring what its docstring claims. The commit count is bounded both sides now and both starvation shapes die (2 commits below the floor, 301 above the ceiling). See DEBT-304. |
| 2026-09-11 | **`13029 passed, 17 skipped, 0 failed in 2654.27s` (rc=0)**, `SUITE TREE STILL` — 58th run, on `24e20a25`, GREEN on the tree carrying A05.1. Launched at the START of the loop and the whole item built off-tree for the 44 minutes. **THE ITEM WAS A05.1'S OWN TAIL, AND BOTH HALVES WERE FOUND BY INSTRUMENTS RATHER THAN REVIEW.** `doc_check` went 0 -> **8 STALE ON ONE COMMIT** because `6d71fdfd` touched `orchestrator.py`, `assembly.py` and `settings.py`; I had marked `document: done` on the strength of the item's OWN document existing. The miss is sharper than forgetting: I ran `docs_touching.py` in the SAME session for the SMALLER change and read its answer, then skipped it for the one touching three of the most-cited files in the tree — because skipping `tests_touching.py` hurts immediately and skipping this costs nothing until a later loop. **So the gate now PRINTS it, as a report that can never fail**, with a guard pinning both that the line stays and that it never acquires `|| fail=1`. **AND `escalation_check` REPORTED EXPIRED ON A QUESTION NOBODY HAD ANSWERED.** ESC-172's premise_check asked whether A05.1's `implement` stage was done — which does not say which bind address he wants. It was testing the BLOCKING CONDITION I had wrongly claimed, and left alone would have dropped a live question from his queue: DEBT-216's shape, second instance. The cause is narrower than "written badly" — the check MIRRORED A DIFFERENT FIELD of the same record (`blocked_until_answered`), and when that field was corrected a loop later, nothing re-read the check derived from it. **SWEPT RATHER THAN ASSUMED: of 85 premise_checks only 3 read an implementation state and the other two are CORRECT**, each expiring on its own answer. One instance, not a class — no guard built, no sweep queued. And the document script split D05.3's existing prose on its first run, inserting a note mid-sentence because the stop condition guessed where the field ended; re-anchored to the header block's end. **Verified BEFORE the gate rather than at it**: the four `Reviewed:` guards were replayed against the copies, including the 8-words-per-sha ratio, which the parser folds the note into — so the ratio is earned, not inherited. |
| 2026-09-11 | **`13030 passed, 17 skipped, 0 failed in 2652.34s` (rc=0)**, `SUITE TREE STILL` — 59th run, on `2d811606`. DEBT-305 built off-tree for the whole 44 minutes, and the item came from REFUSING TO BELIEVE AN INSTRUMENT: `escalation_check` reported ESC-116 EXPIRED at "1 of 20,617 outcomes", and verifying that rather than closing it found the escalation wrong THREE WAYS — while it was asking the operator to consider RETIRING several thousand lines. **It quoted DEAD CONSTANTS** (`_HIGH_THRESHOLD` 0.7 / `_LOW_THRESHOLD` 0.3, measured to appear nowhere but their own definition; the injector consults `DIRECTIVE_LATCH` at 0.62/0.38 since FR-1), **so the count was against the wrong band** (not 1 but **1,595 of 20,617**, on every recent day), **and nobody could have noticed either way** — all three of `inject`'s log calls were DEBUG against **0 DEBUG records in 677,108**. **AND THE GUARD BUILT FOR THAT SHAPE IS BLIND TO IT BY CONSTRUCTION**: `logging_visibility.py` reports a function LOUD on one return and quiet on another, and `inject` is quiet on ALL of them — 49 such functions across the background set, 14 making a decision. **THEN THE FIX'S OWN FIRST VERSION REPRODUCED THE BUG**, caught by running the patched instrument before landing it: the new report was scoped to `BACKGROUND`, which excluded `owls`, so it printed 26/6 and the founding case was ABSENT. Unscoping was measured and rejected (591/182, mostly per-turn paths already observed by their turn), so `owls` joined the set — and its 12 asymmetric findings were TAKEN, not deferred: eleven exempted one at a time, one PROMOTED. **AND THE RATCHET THEN CAUGHT THE STATE MY OWN FIX CREATED**, demanding in writing why `inject`'s quiet branch is deliberate. Two of my own guards were wrong on first run and both would have shipped on a reading — one failed on correct code because a naive `split()` swallowed `main()`. |
| 2026-09-11 | **`13034 passed, 17 skipped, 0 failed in 2646.01s` (rc=0)**, `SUITE TREE STILL` — 60th run, on `efd39833`. DEBT-306 built off-tree for the whole 44 minutes, and the item was a defect in what I shipped the day before: the control plane went out ON and serving, and MEASURED the next morning **every one of these was ZERO** — CLI commands mentioning it, `control_plane` under `cli/`, documents saying where the token lives, `control_plane` under `health/`. A surface nobody could log into, that nothing would notice going dark. **BOTH GAPS WERE NAMED IN A05.1's OWN DESIGN DOCUMENT** under "Not yet built, and named so the gap is legible" — which is HOW THEY SURVIVED: a named gap reads to the next reader as a DECISION, because the record says truthfully that someone thought about it. Naming a gap is not closing it. `census.py` had written the same rule in its own docstring — *"keep REQUIRED_PROBES updated when a consequential default-path subsystem is added"* — and the subsystem was added, default ON, and the set was not updated. **The instrument told its future reader exactly what it could not see, and the reader was me.** **THE CLOSING CHECK WAS WRONG TWICE BEFORE IT WAS RIGHT**, both caught by this file's own rules rather than by the gate: it first grepped `reachability census`, which returns 20 today while the ONLY source literal with that phrase is "audit itself failed — skipping" (it would have counted FAILURES as success); the second draft asked for "6 subsystems reachable", which appears in NO source file because the emitter is a `%d` FORMAT STRING and the evidence-can-exist guard would have refused it. **AND THE FIFTH-SURFACE REPORT FIRED AFTER THE COMMIT FOR THE SECOND LOOP RUNNING** — it prints inside the gate and the gate is chained to the commit, so its finding scrolls past AS THE COMMIT SUCCEEDS. The tool is right; the ordering is mine, and `docs_touching.py` belongs BEFORE staging. It paid for itself anyway: D13.1 counts SURFACES, enumerated six, mentioned the control plane zero times, and states its own invalidation condition in line 10 — a genuine content correction nobody would have reached by remembering. |
| 2026-09-11 | **`13072 passed, 17 skipped, 0 failed in 2667.21s` (rc=0)**, `SUITE TREE STILL` — 64th run, on `802ba3fe`. DEBT-309 built off-tree for the whole 44 minutes: **`/config list` RENDERED CREDENTIALS**, found while measuring A05.2 BEFORE building its `/api/v1/config` route, which is the only reason it was found before seven credential fields went onto HTTP. **FOUR IMPLEMENTATIONS OF ONE QUESTION** — `observability._is_sensitive` (globs + exemptions, logs), `config_helpers` (an opt-in `sensitive=True` MARKER, config), `export/sanitizer._key_is_sensitive` (segment split) and `sandbox/spec.py` (the log predicate plus substrings). Over all **342** fields the name test flags 15, the marker flagged 7, and **the marked set is a strict SUBSET** — the marker never once said something the name does not. `mcp_server.auth_token`'s own description reads *"Sensitive: auto-redacted in logs by the *token key-pattern"*: its author thought about it, wrote down that it was covered, and was right about the logs and wrong about the config surface. **AND A MARKER ALONE WOULD HAVE SAVED NONE OF THE LIST ONES** — `flatten` stopped at `list`, so all EIGHT list-of-model fields were invisible to the walk. **I WROTE THE SEVEN-FIELD MARKING PATCH FIRST AND DISCARDED IT**: that is the symptom repair, and it leaves the next field to memory. **AND THE GATE IS NOW 9:11, NOT ~4.5 MINUTES** — `667 passed, 2 skipped in 551.07s`, against `284 passed in 256.8s` on 2026-09-08. It doubled again in three days, by construction (the marker is the source), and it blew a 600s foreground timeout mid-item. Read it off your own run. |
| 2026-09-11 | **`13041 passed, 17 skipped, 0 failed in 2691.22s` (rc=0)**, `SUITE TREE STILL` — 61st run, on `e724ed17`. DEBT-307 built off-tree for the whole 45 minutes, and the item is THE GAP STATEMENT: an item's `gap` is the sentence that decides what gets BUILT and nothing checked it. **NINE were checked against the tree and SEVEN were wrong or overstated.** The worst is A04.1, a P1 blocking two items, which said no single agent descriptor exists on an evidence line grepping `AgentCard` — **a name I composed** — while `OwlAgentManifest` is read by 29 modules and migration 0118 consolidated an owl's four homes a month earlier; built as written it would have minted a SECOND descriptor. A04.2 said "no lease" while `tasks.lease_owner` + `lease_expires_at` carry a CAS claim with crash reclaim — an instruction to build a second engine. **NOT ONE of the seven was a capability that does not exist**: every one exists and is UNREACHABLE — as a set, from anywhere but a chat prompt, or at a different level than the item names. **AND `/usr/bin/grep -rl` WALKS `__pycache__`**, which inflated one count 29 -> 57 and listed three `.pyc` files as evidence; this harness's `grep` excludes them. The two disagree on the SET, which this file records for order and for truncation but not for membership. **THE DERIVED PATH THEN CAUGHT A GUARD THAT PASSED OFF-TREE**: the new test file resolved `parents[2]`, one level too many, and the scratchpad's extra level happened to hold a `progress.yml` and a `scripts/` — six tests green there, six `FileNotFoundError` here. It sits directly in `tests/`, so no package path and no tripwire reaches it and only the 45-minute run would have. **And `tests_touching.py` could not see it either**, because `git diff` does not list an untracked file — the new-module case, answered with silence. `docs_touching.py` had the `ls-files --others` clause all along: two copies of one rule, the wrong copy gating the tests. Fixed and mutation-proven. |
| 2026-09-12 | **RED — `1 failed, 13147 passed, 17 skipped in 2643.71s` (rc=1)**, `SUITE TREE STILL` — 70th run, on `dfe04ede`, and the failure was MINE from four hours earlier: DEBT-310 added `control_plane.password = "admin"` at the operator's request, `gen_config_example.py` emits every default VERBATIM, and `test_no_secret_value_reaches_the_artifact` found `control_plane.password = admin` in `docs/stackowl.yaml.example`. **THE ROOT CAUSE IS A GUARD OVER AN EMPTY POPULATION, not the default.** That file's own header has said *"Secrets do NOT belong in this file. Store a REFERENCE instead"* since D18.2 — the rule lived in PROSE and nothing enforced it, so "no secret reaches the artifact" was true by luck: MEASURED, **13 of the 215 emitted fields are credential-named and TWELVE defaulted to `""`**. The thirteenth was always going to break it. DEBT-300's lesson eight days on and DEBT-299's before it, both of which concluded that a currently-empty population is pinned only by one you CONSTRUCT; this one had neither a rule nor a construction. The generator MASKS them now (`password: keychain:stackowl-control-plane-password`), decided by `is_credential_name` — the same predicate DEBT-309 reduced four vocabularies to — which also improves the twelve, since `bot_token: ""` taught an operator nothing. **AND THE GATE COULD NOT HAVE CAUGHT IT:** four tests in that file, only TWO marked `@pytest.mark.tripwire`, and it sits DIRECTLY in `tests/` where no `tests/<package>` path reaches it — a 0.18-second assertion reachable only by a 44-minute run. Nothing recorded a reason for the omission, so it reads as oversight; the marker is on. The test also carried a FOURTH copy of the credential vocabulary as a hand-written word list inside the guard, and its assertion is now stronger, not weaker: "non-empty" passed the twelve `""` lines, "starts with a reference scheme" fails those too. See DEBT-311. |
| 2026-09-12 | **`13202 passed, 17 skipped, 0 failed in 2643.33s` (rc=0)**, `SUITE TREE STILL` — 72nd run, on `5521a448`, GREEN on the tree the previous loop shipped. DEBT-314 built off-tree for the whole 44 minutes, and **THE PREMISE THE ITEM OPENED WITH WAS REFUTED BY ITS OWN MEASUREMENT — for the second time this week.** The aggregator's double-timeout branch returned `status="down"` while the comment directly beneath it said the case is *"most likely to be about the HOST rather than the subsystem"*, so I set out to make all 40 of them `unknown`. Joining each double timeout to its own sweep's `collect: exit` line killed that: 40 timeouts across 29 sweeps, and **24 fired while they were the ONLY non-ok subsystem** — fourteen other contributors answered inside the same window. A busy box does not answer fourteen probes and drop one. Only 11, in three sweeps of 2/4/5, have the host shape. **THE TELL WAS TWO EXISTING TESTS I WOULD HAVE HAD TO EDIT** (`test_a_subsystem_that_is_REALLY_gone_is_still_reported_down`, `test_liveness_still_falls_for_a_subsystem_that_is_really_down`), and the recorded decision they carry is right: shipping my version would have retired the detection of the exact failure `is_live` exists for. The rule became CORROBORATION — a non-answer keeps `unknown` only when something ELSE also failed the same sweep — and both tests pass untouched. **The cause underneath was FIVE COPIES AND NO NAME**: `Literal["ok", "degraded", "down"]` written out independently in five modules, so a fourth word was a five-file edit rather than a design decision — and the PARTITION was implicit in four readers, none of which raises on a word it does not know, so a new state would have been counted HEALTHY by `health_sweep` and shown as an outage by the CLI, silently. **AND TWO CONTROLS FAILED FOR CORRECT WORK:** `test_the_scan_can_actually_fail` demanded `>= 10` live sites and this took it to 9 — a floor under a population the work exists to change, second instance of the 49th run's shape, now proven against a population it CONSTRUCTS. And my own `closing_check` printed CLOSEABLE on ZERO records because **`log_since.sh` returns a COUNT and I piped it to `wc -l`** — second instance of assuming that script's signature instead of reading it, both failing in the direction that CLOSES work. Refused structurally now, with a control that runs the real script to prove the pipe really would read 1. |
| 2026-09-12 | **`13213 passed, 17 skipped, 0 failed in 2777.21s` (rc=0)**, `SUITE TREE STILL` — 73rd run, on `346a57be`. A05.10 step B1 — the five-destination app shell — was measured, built, mutation-proven and run HEADLESS off-tree for the whole 46 minutes, against a scratch copy of `page.py` injected into `sys.modules` so the real tree never moved. **AND GROUPING THE PANELS IS WHAT EXPOSED THE ITEM'S REAL DEFECT: the ORDER of `PANELS` was a sixth enumeration wearing no list.** `load()` built its status line from `results[1]` and `results[2]`, correct only while that array stayed in the order it was first typed in — and destinations reorder it, so the status line would have reported the config payload's `schedules`. **NO TEST COULD HAVE SEEN IT**: the executing harness answered EVERY route with one shared payload carrying every shape, so both readings produce the byte-identical `0 schedules · 0 settings`, MEASURED by running both against the old fixture. A fixture that cannot show the bug proves nothing; per-route counts went in FIRST and only then does reverting fail anything. **The shell needed no edit to four behavioural guards, and that was a design choice rather than luck**: a wrapper's `hidden` means "not the screen you are on", a section's means "this panel has no data", so `test_a_healthy_platform_renders_every_section` still asserts all eight and still means it — navigating by toggling `section.hidden` would have left all four passing while asserting something weaker. **AND MY OWN DESIGN DOCUMENT WAS WRONG TWICE BEFORE IT SHIPPED, caught by running its Verification section rather than trusting it**: it cited `test_the_page_makes_no_external_request`, a name I composed (the real one is `test_nothing_is_fetched_from_another_host`), and its no-external-reference check matched a bare `//` — which also starts a JavaScript comment — so it reported EXTERNAL REFERENCE against a clean page. **A05.9 then claimed "SIX tables" against a measured NINE**, the THIRD time that one sentence went stale (its own header records two->three and four->five); the count was REMOVED rather than corrected a third time, because a prose count beside a page that owns the count is two copies of one rule. The item's `closing_check` was also asking for `data-label` — the responsive-table trick this item's own brainstorm MEASURED AND REFUTED — so it could only ever have closed on the wrong design. |
| 2026-09-12 | **`13220 passed, 17 skipped, 0 failed in 2671.93s` (rc=0)**, `SUITE TREE STILL` — 74th run, on `93ae9b9f`. DEBT-315 built off-tree for the whole 45 minutes, and **THE ITEM WAS FOUND BY THE RULE THIS FILE ALREADY STATES FIVE TIMES: break a window down by DAY before believing it is current.** Measuring the live payloads for A05.10's synthesis layer, `GET /api/v1/tasks` returned 82 rows with 77 `dead_letter` — a platform whose work is 94% dead. By day: **72 of the 77 were created on 2026-08-20**, twenty-three days earlier, in ONE batch; today 8 completed and 0 dead. The panel was reporting August. **THE CAUSE IS THE WORD'S AGE:** `dead_letter` arrived with migration 0119 after `completed`/`failed` had been the terminal pair for months, so everyone who writes the set from memory writes TWO words — four live instances, including the DENOMINATOR (`unseen_other_owner`, which would have counted other owners' dead letters against a set that drops mine) and the page's own note to the operator. **THREE OF THE FILES ALREADY SAID THE ANSWER**: `activity.py`'s docstring says a set view "whose first screen is last month's failures answers the wrong question" and it was serving exactly that; its `DEFAULT_LIMIT` comment says "the live population is small (5 today)" beside a read returning 82; `_handle_tasks`'s docstring, written the same morning, describes a panel of five. Every sentence correct, none of them a predicate. **AND THE TEST THAT SHOULD HAVE CAUGHT IT AGREED WITH IT BY CONSTRUCTION** — it seeds `completed` and `failed`, the same two words as the filter it checks, so it could never have disagreed; superseded by one seeding ONE ROW PER DECLARED STATUS from `get_args(TaskStatus)`. **THEN MY OWN NEW GUARD MATCHED MY OWN PROSE, the FIFTH instance this session**: it read LINES, so the three comments explaining the two-word predicate were themselves offenders. It asks the AST now — `#` comments are not in the tree and a docstring is the one string constant that opens a body. **And a test double answered `{"n": 3}` to ANY `COUNT(*)`**, so a second aggregate raised `KeyError`; it derives its shape from the statement's own `AS` aliases rather than having the canned row patched. Live: 82 -> 5, and the five are byte-for-byte the rows the route's docstring describes. |
...
owner-scope allowlist. The gate is **~9 to 10 minutes** — MEASURED across THREE runs on
2026-09-12: `748 passed, 2 skipped, 12428 deselected` in **551.04s, 552.24s and
595.66s** (the longest overlapped other work on this box). The history of this ONE
NUMBER is the argument for re-reading it: ~40s, then `138 passed` in 105.88s
(2026-09-06), then `284/284/284/286/290 passed` in 256.8-265.9s (2026-09-08), then
`667 passed` in 551.07s (2026-09-11), now 748. **It has grown roughly fifteenfold, and
every time it was wrong here it was wrong LOW** — which is the dangerous direction,
because a gate budgeted at 4.5 minutes and taking 10 is a gate someone kills. Note the
COUNT grew 667 -> 748 in one day while the DURATION did not move: the marker is the
source, so every loop that ships a guard adds to it and nothing updates the figure.
Read it off your own run rather than trusting this sentence.
