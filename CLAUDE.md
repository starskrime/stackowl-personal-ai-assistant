...
**A full `pytest` run does NOT hang — it takes ~45 minutes, and it is GREEN.**
`./scripts/full_suite.sh` runs it detached and the log ends with its own verdict,
so collecting it is `grep -c 'SUITE DONE'` (0 = running, 1 = finished) rather than a
guess at a process table. Measured 2026-09-12: `13226 passed, 17 skipped, 0 failed
in 2711.40s` (rc=0). The per-run history that used to sit here was a log of loop
invocations and went with the loop.
...
**Every moment in time is stored as ISO-8601 TEXT in UTC.** Migration 0029 is where
the project settled on it; the 28 columns older than that are the legacy epoch era and
are named by `tests/db/test_time_is_stored_one_way.py` so nobody has to guess which.

**AND A STORED TIMESTAMP IS NOT COMPARABLE TO `datetime('now', …)`.** SQLite's own
function returns `2026-09-05 15:07:19` — space-separated — while every column here
holds `2026-09-05T00:00:50.784648+00:00`. Compared as STRINGS `'T'` (0x54) sorts after
`' '` (0x20), so **every row sharing the bound's date passes the filter whatever its
clock says**. MEASURED 2026-09-12: a 24-hour window counted a 30-hour-old run, and it
hid on live data because most rows genuinely are recent — 5,195 against a true 5,188.
Bind a Python-computed ISO string as a parameter instead. Six other sites in `src/`
still compare the old way and are lenient by up to a day.
