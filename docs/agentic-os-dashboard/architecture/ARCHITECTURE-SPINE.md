...
- **Rule:**
  ...
  - Spike B1 is the gate: private-CA trust, passkeys on the local name, and WebTransport certificate hashes, on stock iPhone, Android and desktop Chrome. No fallback is designed; a B1 failure goes to the owner as an Open Question.
...
- **Rule:**
  ...
  - An unreadable identity store is never treated as "no owner": sign-in is refused with a remedy, the lookup is retried on each sign-in, and the failure is reported to self-healing as an incident (L3). Enrolment and recovery follow AD-37.
...
- **Rule:**
  ...
  - `ConsentRequestFrame` carries `reply_target` across IPC. The voice channel's prompter routes to on-screen approval. A Telegram answer resolves only consent Telegram itself could request; an item that needs a signed tap (AD-27) resolves only on a Bridge device. Device approvals are never mirrored.
...
- **Prevents:** a single delivery path that fails silently; device requests announced on Telegram; duplicate Telegram messages; stale notifications after resolve; a notification that dead-ends off the home network; content cached on the phone; SSRF through a push endpoint.
- **Rule:**
...
- **Rule:**
  ...
  - Completed COMMAND rows are kept at least as long as the undo window and journal retention (AD-4), exempt from the shorter task prune.

### AD-27 — One action-policy gate in `authz/` [ARCHITECT]

- **Binds:** the AD-1 execution path, voice orders and approvals, scheduled and autonomous runs, the Needs-you strip, standing authority, §8 item 12.
- **Prevents:** each surface deciding its own confirmation; a misheard or injected voice order running an irreversible action; a replayed or forged tap; irreversible autonomy nobody set up; an owl granting itself authority.
- **Rule:** One gate in `authz/` decides every command from its declared reversibility (AD-26) and its requester kind: owner (a signed-in Bridge device or the owner's allowlisted chat), owl or crew, `voice-unverified`, or autonomous (Q11, Q16, Q35, Q36, Q49, Q52).
  - The owner's own order for a reversible action runs at once, with undo and no read-back.
  - An owl or crew request needs a read-back first.
  - `voice-unverified` may trigger only reversible commands. Irreversible actions and approvals from voice need an on-screen signed tap. Owl-authority grants never come through voice.
  - An irreversible action, an approval, an owl-authority grant and a device approval need a read-back plus an on-screen signed tap. The tap is signed by the device's non-extractable key (AD-16) over a server-issued, single-use nonce bound to the command digest (type, payload, target, Needs-you item id and version), and expires quickly. The read-back is rendered deterministically from the payload by the narrator, never by a model.
  ...
  - Standing authority lives in one `authz/` table keyed by command type and scope, written only by `authority.grant` and `authority.revoke` commands that need a signed tap. Session consent grants stay in memory and never satisfy the irreversible rule.
...
- **Rule:**
  - Setup mode is possible only while zero passkeys exist. A setup code is issued only at boot or by the host CLI, never by a network request. It goes to the terminal and to the single allowed Telegram user only on a first install where no owner record ever existed. A damaged store never re-enables Telegram enrolment: sign-in is refused, and only a terminal-only code from the host CLI, the recovery code or a signed-in device can enrol. Setup codes are accepted only from the home network. Pre-auth responses reveal setup state only inside the setup page flow.
  ...
  - Device approval anti-phishing: the requesting device shows its device name and a short matching code, and the approving Bridge screen shows the same name and code. The owner confirms the match with a signed tap (AD-27). Only one device request may be pending at a time, and it expires with its item.
...
| Cross-cutting: auth | Device-bound token with proof of possession on every request and stream (AD-16). Severity, action policy and consent run once in the core execution path (AD-1, AD-18, AD-27). Irreversible actions, approvals, owl-authority grants and device approvals need a signed tap (AD-27). Strict CSP on every response (AD-36). |
...
| Station — Missions | `web/bridge/`, task and job events, jobs with next due time from the snapshot; retry, cancel, pause, run-now, steer and take-over are COMMAND types declared by the Missions epic | AD-1, AD-2, AD-26, AD-29, AD-34 |
...
| Station — Security | `web/bridge/`, `authz/identity/` devices, sessions and worker tokens, device approval, consent and approval events, `audit_log` security evidence | AD-16, AD-17, AD-18, AD-23, AD-37 |
| Needs-you strip | durable Needs-you items with one resolver and the attention policy in `journal/`, channel prompters | AD-5, AD-18, AD-27, AD-28 |
...
| Anticipation and undo | COMMAND tasks with declared reversibility and undo, the action-policy gate, `utterance_id` undo | AD-1, AD-26, AD-27 |
...
| Phone notifications | the gateway notifier, Telegram, Web Push and the Bridge service worker; at home a tap opens the item, away from home the service worker shows a cached metadata-only summary with "open at home" and a link to Telegram | AD-5, AD-19, AD-28, AD-30 |
...
1. **Separate OS user for the gateway:** the agent's unattended `shell` runs as the same OS user as the gateway, so it can reach the SQLite identity store and the secret-store file fallback at rest. AD-27's tripwire covers agent tools, not the shell. Should the gateway (identity store, server certificate key, VAPID key) run as its own OS user? Owner decision.
