...
**For** Boss. **Approved** by Boss on 2026-09-12. **Date** 2026-09-12. **Amended** by Boss's decisions from the architecture run on 2026-09-13 (A1–A12, §12). **Status:** approved decision document; nothing is built. Product decisions come from Q1–Q52, L1–L4 and A1–A12 (§12). Anything else is marked *proposal*, *finding* or *open question*.
...
4. **One mind, many surfaces.** One conversation, memory and task loop everywhere. Web actions become tasks in the existing loop, never a second engine [02 hard fact 14]. The Bridge is reachable only on the home network, where it is the full-control surface; away from home Telegram is the conversation surface, can answer approvals (A8) and carries notifications, alongside Web Push (whose tap shows the cached, metadata-only summary, A5), and there is no Bridge control (A1, A5, A8, architecture run, 2026-09-13).
...
6. **Permissive licences only:** MIT, Apache, BSD; CC-BY for weights; SIL OFL for fonts (A6, architecture run, 2026-09-13). The default install contains nothing else. NVIDIA-licensed models are never bundled; on NVIDIA hardware the owner may choose to download one after its licence is shown (Q44). *(Superseded by owner decision, 2026-09-14: no licence rules; see A13.)*
...
**Postures.** Desktop shows the full bridge. The phone opens on the compact viewscreen plus the Needs-you strip, with Comms one swipe away; opening from a notification at home goes straight to that item (Q8, Q38); away from home it shows a cached summary instead (§3.3; A5, architecture run, 2026-09-13). Station access (A10, architecture run, 2026-09-13): on desktop the six station names sit on the outer rim of the Helm Dial, the approved Viewscreen design, and open the station sheet, with the Crew panel and Ship's log beside the dial; on the phone a bottom bar holds the six stations plus Now and Log; and every mark also opens its own station record.
...
| **Crew** | Each owl's activity, authority (bounds ∩ ceiling), skills, DNA; pause, resume, rename, grant, take over its mission (A9) | `owls` (11), `owl_dna`, `skill_ownership`, `authz/bounds.py`, `owl_build` (grant is always-ask) | No HTTP action; no activity events |
| **Missions** | Tasks, schedules, retries, dead letters; pause, steer, take over, retry, cancel, run now. Take over pauses the owl's mission and hands the owner its current plan to edit and resume, or to cancel; it never continues silently (A9, architecture run, 2026-09-13) | `tasks` (1,221), `TaskLoop`, `jobs` (170), `job_runs`, `JobScheduler` pause/resume/run_now | Nothing cancels or retries a task; `steer`/`stop` frames never sent; no finished-task history |
...
- **Anticipation.** Owl acts alone only within authority already granted, shows what it did, offers undo, and turns anything beyond that into a Needs-you request. Undo stays on the action's card until the action is superseded, meaning a later change to the same target, or 24 hours pass, whichever comes first (A9, architecture run, 2026-09-13). It takes an irreversible action on its own only when the owner explicitly set that up beforehand, such as a scheduled daily report that sends a message; each run leaves a tappable record. Any other irreversible action goes to Needs-you. When Owl or a crew member asks for a reversible approval that is not of consequential severity, a spoken "yes" counts only after Owl reads the request back, and the card then shows undo (A7, architecture run, 2026-09-13); the owner's own spoken orders for reversible actions run at once, with undo, unless they are of consequential severity, when Owl reads them back and they require the signed on-screen tap in the Bridge or an explicit Telegram approval (Q36, Q49; A12, architecture run, 2026-09-13). In the platform, authority is owl bounds ∩ creation ceiling plus consent grants that die on restart by design [02 hard facts 5–6]. *Finding:* there is no general undo; some actions have an inverse (pause/resume), many do not (a sent message).
...
- **Voices.** A distinct bundled voice per owl, Owl's the most distinctive; no cloning. *Proposal:* Kokoro-82M (Apache-2.0) voice packs by default, Pocket TTS (CC-BY-4.0) for GPU-less hosts [03 §3].

---

## 5. Voice

**Modes.** Push-to-talk everywhere; hands-free as a toggle (open mic with voice-activity detection, end-of-turn detection and barge-in [03 §4]). No wake word. The conversation is one word away only while hands-free is on (Q37). A mic-live indicator whenever audio is captured [01 §5.2]. Transcripts appear live and keep auto-sending. Because a transcript can be misheard, the owner's own spoken order can at most trigger a reversible action, which runs at once, shown with undo and with no read-back, unless it is of consequential severity: then Owl reads it back and it requires the signed on-screen tap in the Bridge or an explicit Telegram approval, even when reversible (A12, architecture run, 2026-09-13); irreversible actions, consequential approvals, owl-authority grants and new-device approvals require the signed on-screen tap in the Bridge or an explicit Telegram approval, and a spoken yes after read-back approves only requests that are reversible and not consequential (Q35, Q49; A7, A8, A11, architecture run, 2026-09-13). When the phone is backgrounded the browser cuts the mic [03 §5.3], so hands-free counts as paused; reopening takes one tap, which also starts the spoken briefing (Q37). English only.
...
**Orders, approvals and irreversible actions.** Voice can request anything (Q16). The owner's own spoken order for a reversible, non-consequential action runs at once, with undo and no read-back (Q35); a consequential one gets Owl's read-back and then requires the signed on-screen tap in the Bridge or an explicit Telegram approval, even when reversible, because severity outranks reversibility for orders and requests alike (A12, architecture run, 2026-09-13). When Owl or a crew member asks the owner for something, Owl reads back exactly what will happen (Q36). Severity wins over reversibility: irreversible actions, consequential approvals (any request of consequential severity, even when reversible), owl-authority grants and new-device approvals require the signed on-screen tap in the Bridge or an explicit Telegram approval, the tap cryptographically signed by the signed-in device; a spoken yes after read-back approves only requests that are reversible and not consequential (Q16; A3, A7, A8, A11, architecture run, 2026-09-13). Q49 sets this split. Owl acts irreversibly on its own only where the owner set it up beforehand (§4). *Finding:* the platform has no "irreversible" class, only always-ask consent categories and a `destructive` flag on command actions [02 §4.1, §4.3]. If a transcript was misheard and the order already ran, the owner's correction makes Owl undo that action automatically, show the undo on its card, and then carry out what the owner actually said (Q52).
...
**Sign-in.** Passkeys plus a one-time recovery code. At setup the platform shows a one-time setup code in its terminal and sends it to the owner's existing Telegram; entering it registers the first passkey. "The owner's Telegram" means the single allowed Telegram user. *Finding:* today that is `telegram_channel.allowed_user_ids` holding exactly one id, resolved by `resolve_owner_addresses` (`notifications/recipient.py:28-64`). With none or several ids, codes are shown only on the terminal/CLI; the bridge's owner record (§8 item 7) formalises this later. A new device is approved from an already signed-in dashboard device, as a Needs-you item, with the recovery code, or from Telegram, where the message shows the device name and the same short matching code displayed on the new device (Q41; A11, architecture run, 2026-09-13, superseding Q48's "never from Telegram"). Residual risk, stated: a hijacked Telegram account plus home-network access could enrol a device and gain full Bridge control. An unreadable identity store (backend I/O error, locked keyring, permission failure) never enters setup mode: sign-in is refused with a remedy, retried on the next attempt and reported to self-healing. A damaged store (bytes readable but failing schema or integrity validation) returns the install to setup mode, and that setup code is shown only on the host terminal or by `stackowl control-plane reset-password`, never sent to Telegram, so re-enrolling proves presence at the host; ordinary new-device approvals from Telegram are unaffected (L3, L4, A11). The setup code is the same ownership proof the current dashboard gains in L1, so the bridge reuses that mechanism rather than inventing a second one.

**Device-bound sessions** (A4, architecture run, 2026-09-13). Each signed-in device gets a session token bound to a non-extractable key created in its browser, so a copied token is useless on any other device. A strict content-security policy applies, tokens have a lifetime cap, and token reuse is detected. Irreversible actions, consequential approvals, owl-authority grants and new-device approvals require the signed on-screen tap in the Bridge or an explicit Telegram approval, the tap cryptographically signed by the signed-in device; a spoken yes after read-back approves only requests that are reversible and not consequential (A3, A7, A8, A11, architecture run, 2026-09-13).

*Finding:* today one shared admin/admin login on `0.0.0.0` over plain HTTP makes every caller the default principal with every severity; no users, password hashing or session store exist [02 §1.4, §6]. The Q29 fix (L1–L4, §12, in progress separately) changes the login itself: no `admin/admin` and no token for a publicly known password; ownership proven by the setup code plus a new password; the password removed from `stackowl.yaml` and stored once as a salted hash (a custom YAML password is imported once on upgrade); an unreadable store (backend I/O error, locked keyring, permission failure) refuses sign-in with a remedy, never enters setup mode and is reported to self-healing as an incident, and a damaged one (bytes readable but failing schema or integrity validation) returns to setup mode with a setup code shown only on the host terminal; and `stackowl control-plane reset-password` on the host issues a fresh setup code, shown only on the host terminal, without a restart. It does not add HTTPS, per-user principals or severity checks; those stay bridge prerequisites (§8 items 2, 5, 7).
...
7. **★ Owner identity.** A real owner record that formalises today's single-allowed-Telegram-user rule (§7), the setup-code ownership proof shared with L1, passkeys, new-device approval from a signed-in dashboard device, the recovery code or Telegram (A11, architecture run, 2026-09-13), sessions with expiry and revocation, and web sessions mapped to the owner's principal and identity alias, so the web is the same person as the Telegram owner [02 §6].
...
S1–S9 record P50/P95 [03]. *(Amended by owner decision, 2026-09-13, during epics: voice runs on the platform's own box, as it will on any customer's clone. S1–S4, S6, S8 and S9 ship as the built-in `stackowl voice check` that every install runs on its own host; S5 and S7 are a Bridge device check page on the owner's phones. The owner's run on this box is the reference verdict for defaults. Speech models may run on the Jetson dev box. Supersedes the earlier four-host, not-the-dev-box wording.)*
...
| **S4** TTS first audio + owner's blind ranking | ≤ 250 ms GPU / 600 ms Mac / 1 s CPU; an engine in his top two | Neither met | Default voice set (Q30) |
...
| **S8** Probe the platform's own host (every install, via `stackowl voice check`), then force-degrade | Right tier; honest fallback to push-to-talk | Wrong tier or silent failure | Q27 ladder |
...
| **B1** (re-scoped: A1, A2, architecture run, 2026-09-13) Fresh clone, no domain, home network over IPv4, stock iPhone, Android and desktop Chrome: trust in the private per-install certificate authority with guided steps and a CA fingerprint check, the local host name, setup code (to the single allowed Telegram user; terminal only with none or several), open app, passkey on the local name, a second device approved from the first, WebTransport with a short-lived certificate identified by fingerprint, mic, PWA install, push | Works on all three with guided steps only; a device approval in Telegram shows the device name and the matching code shown on the new device (A11) | Private-CA trust, a passkey on the local name or a WebTransport certificate fingerprint is unworkable on any of the three, or a router change is needed; the owner then decides what to do (A2) | Q17, Q41, A1, A2, A11 |
...
10. **Mirrored approvals** (A8, architecture run, 2026-09-13): decided. Telegram buttons may approve any request, irreversible and consequential ones included, from anywhere; the Telegram message carries the same read-back text as the Bridge, and the first answer wins against the strip. A hijacked Telegram account can therefore approve irreversible actions, a stated residual risk. New-device approvals are mirrored too, with the device name and matching code (A11, architecture run, 2026-09-13).
...
| Q4 | Surfaces | One mind, many surfaces; bridge alone has full control; amended 2026-09-13 (A1): the Bridge is the full-control surface at home, away from home Telegram is the conversation surface, can answer approvals (A8) and carries notifications, alongside Web Push (tap shows the cached, metadata-only summary: A5), with no Bridge control |
...
| Q8 | Postures | Phone = comms, desktop = full bridge (phone opening: Q38; station access: A10) |
...
| Q11 | Anticipation | Acts within authority, shows it, offers undo; asks otherwise (irreversible: Q36; undo lifetime: A9) |
...
| Q16 | Voice authority | Request anything; irreversible needs read-back + tap (orders vs requests: Q49; re-confirmed 2026-09-13, tap signed by the signed-in device: A3); amended 2026-09-13 (A8, A11): irreversible actions, consequential approvals, owl-authority grants and new-device approvals require the signed on-screen tap in the Bridge or an explicit Telegram approval; a spoken yes after read-back approves only requests that are reversible and not consequential |
...
| Q20 | Layout | Viewscreen + six stations + Needs-you strip (station access: A10) |
| Q21 | Interrupts | One queue everywhere + phone notification; pause/steer/take over, never forced (new-device approvals: A11, superseding Q48); amended 2026-09-13: Telegram buttons may approve any request, first answer wins against the strip (A8); take over pauses the mission and hands the owner its plan to edit and resume, or cancel (A9) |
...
| Q25 | Licences | Permissive only; replace Piper; NVIDIA optional extra (refined by Q44; fonts under SIL OFL: A6) *(superseded by A13)* |
...
| Q35 | Misheard voice | Transcripts keep auto-sending; the owner's own voice order triggers at most a reversible action, at once, with undo, no read-back (Q49), unless it is consequential, when it gets read-back and requires the signed on-screen tap in the Bridge or an explicit Telegram approval (A12); irreversible actions, consequential approvals, owl-authority grants and new-device approvals require the signed on-screen tap in the Bridge or an explicit Telegram approval; a spoken yes after read-back approves only requests that are reversible and not consequential (A7, A8, A11) |
...
| Q41 | First passkey, new devices | Setup code in terminal + owner's Telegram (the single allowed Telegram user; terminal only if none or several) registers the first passkey; new device approved from a signed-in dashboard device (Needs-you), with the recovery code, or from Telegram showing the device name and matching code (amended 2026-09-13: A11, superseding Q48) |
...
| Q44 | NVIDIA-licensed models | Never bundled; permissive-only default install; owner may download on NVIDIA hardware, licence shown first *(superseded by A13)* |
...
| Q48 | New-device approval | Only from an already signed-in dashboard device or with the recovery code; never from Telegram, though other approvals are mirrored there; superseded 2026-09-13 (A11): also from Telegram, where the message shows the device name and the same short matching code displayed on the new device |
| Q49 | Voice orders vs voice approvals | Owner's own order for a reversible action runs at once, with undo, no read-back (a consequential spoken order: A12); a request from Owl or crew needs read-back, after which a spoken yes approves it if reversible and not consequential (re-affirmed 2026-09-13: A7); irreversible actions, consequential approvals, owl-authority grants and new-device approvals require the signed on-screen tap in the Bridge or an explicit Telegram approval; a spoken yes after read-back approves only requests that are reversible and not consequential (A8, A11) |
...
- **L3:** an unreadable password store (backend I/O error, locked keyring, permission failure) is never treated as "no password" and never enters setup-code mode. The lookup is retried on each sign-in, sign-in is refused with a remedy (503), and the failure is reported to self-healing as an incident. A damaged record (bytes readable but failing schema or integrity validation) returns the install to setup-code mode, and that setup code is shown only on the host terminal or by `stackowl control-plane reset-password`, never sent to Telegram (proof of presence at the host).
- **L4:** `stackowl control-plane reset-password` on the host clears the password and issues a fresh setup code, shown only on the host terminal and never sent to Telegram, without a restart.
...
- **A3 Confirmations:** Q16 re-confirmed, as amended by A8 and A11: irreversible actions, consequential approvals, owl-authority grants and new-device approvals require the signed on-screen tap in the Bridge or an explicit Telegram approval, the tap cryptographically signed by the signed-in device; a spoken yes after read-back approves only requests that are reversible and not consequential (A7).
...
- **A7 Voice approval:** Q49 stands and is re-affirmed: after Owl reads back a request from Owl or a crew owl that is reversible and not of consequential severity, a spoken "yes" approves it, even though a voice is not a verified identity, and undo shows on its card; severity wins over reversibility, so irreversible actions, consequential approvals (any request of consequential severity, even when reversible), owl-authority grants and new-device approvals require the signed on-screen tap in the Bridge or an explicit Telegram approval (A8, A11); a spoken yes after read-back approves only requests that are reversible and not consequential.
- **A8 Telegram approvals:** Telegram buttons, as the owner's authenticated channel, may approve any request from anywhere, so irreversible actions, consequential approvals, owl-authority grants and new-device approvals require the signed on-screen tap in the Bridge or an explicit Telegram approval, and a spoken yes after read-back approves only requests that are reversible and not consequential; the Telegram message carries the same read-back text as the Bridge; the first answer wins against the Needs-you strip; residual risk, stated: a hijacked Telegram account can approve irreversible actions; new-device approvals in Telegram follow A11 (supersedes Q16's on-screen tap for approvals answered in Telegram; amends Q21 and §11 question 10).
- **A9 Undo and take over:** undo stays available on an action's card until the action is superseded (a later change to the same target) or 24 hours pass, whichever comes first; "take over" pauses the owl's mission and hands the owner its current plan to edit and resume, or to cancel, never silent continuation (defines Q11's undo and Q21's take over; §3.2, §4).
- **A10 Station access:** on desktop the six station names sit on the Helm Dial's outer rim and open the station sheet, with the Crew panel and Ship's log beside the dial; on the phone a bottom bar holds the six stations plus Now and Log; every mark also opens its own station record (amends the Q8 and Q20 postures, §3).
- **A11 New-device approval via Telegram:** a new device may be approved from an already signed-in Bridge device, with the recovery code, or from Telegram, where the message shows the device name and the same short matching code displayed on the new device; residual risk, stated: a hijacked Telegram account plus home-network access could enrol a device and gain full Bridge control; a damaged identity store still returns the install to setup mode with a setup code shown only on the host terminal, never sent to Telegram, per L3 and L4 (supersedes Q48's "never from Telegram"; amends Q41, §3.3, §7, §8 item 7, §9 B1, §11 question 10).
- **A12 Consequential spoken orders:** the owner's own spoken order for a consequential action gets Owl's read-back and then requires the signed on-screen tap in the Bridge or an explicit Telegram approval, even when reversible; everyday reversible, non-consequential spoken orders still run at once with undo (Q35); severity outranks reversibility for orders and requests alike (amends Q35, Q49; §4, §5).
- **A13 Licence rules dropped (epics run, 2026-09-14):** "This is open source platform. No license". There is no licence allow-list; Piper, python-zeroconf, pywebpush and NVIDIA-licensed models are ordinary engineering choices. Hash-pinned downloads and no-pickle weights stay as security rules. Supersedes principle 6, Q25, Q44 and A6's licence framing.
