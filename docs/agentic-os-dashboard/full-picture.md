...
**For** Boss, to approve. **Date** 2026-09-12. **Status:** decision document; nothing is built. Product decisions come from Q1–Q49 and L1–L4 (§12). Anything else is marked *proposal*, *finding* or *open question*.
...
2. **Dark-cockpit attention.** Healthy is calm, and normal activity moves in cream light only. The accent colour appears only when something needs the owner, and is never diluted: a failure Owl could not heal becomes a Needs-you item at higher intensity, while a failure Owl healed shows only in its record (Q34). Strong motion and sound belong to what needs the owner; severity sets intensity, not activity volume [01 §3.4]. This governs unsolicited sounds; a sound that answers something the owner just did, such as the acknowledgement sound (Q39), is feedback, not an alert.
...
**Stop, steer, correct.** No keyword matching. The instant the owner starts to speak, Owl's speech pauses; the utterance then goes through Owl's normal understanding, which decides whether it stops the task, steers it or corrects the last transcript (Q40). If the utterance is a steer, a stop or a new question, the paused speech is dropped; if it was only a backchannel ("mm-hm") or noise, Owl resumes where it left off (Q47).
...
**Long tasks.** Every tier gives a non-spoken acknowledgement within ~300 ms, measured from the moment end of turn is detected, not from the last spoken syllable: the owl mark switches to "thinking" and a soft sound plays. The spoken "on it" follows as fast as the hardware allows, under a second on GPU and Mac (Q39). After that Owl speaks only at meaningful milestones. Speaking over Owl pauses its speech, never the task by itself; Owl's understanding decides whether the words stop or steer the task (Q40), and so whether the paused speech is dropped or resumed (Q47). *Findings:* progress chunks carry only step name, index and total, too little for a meaningful milestone [02 §5.2]; the `stop` and `steer` frames exist but are never sent [02 §4.2].
...
- **speaking:** follows Owl's output audio; pauses the instant the owner starts to speak, picks up where it left off after a backchannel or noise, and ends when the paused speech is dropped (Q40, Q47)
- **needs you:** the accent colour, at higher intensity for a failure Owl could not heal
- **stale or disconnected:** stops breathing and shows the age of the last event

**Sound.** Subtle event sounds on by default, with volume control; they duck under Owl's voice and respect silent mode. One sound per kind of event, never a barrage [01 §3.2 #8]. A soft sound joins the thinking acknowledgement (Q39). It answers something the owner just did, so it is feedback, not an alert; principle 2 governs unsolicited sounds.
...
11. **A streaming voice path:** mic → streaming STT → pipeline → streaming TTS, spoken progress, speech that pauses when the owner talks and is then dropped or resumed (Q47), and wired `stop`/`steer` frames chosen by Owl's understanding, not keywords (Q40). ★ Replacing GPL Piper stands alone [03 §7].
...
| **S7** Installed PWA: permissions, lock, app switch, headset | 10 min, no re-prompt; announced pause/resume; after backgrounding, hands-free shows as paused and one tap resumes it and starts the briefing | Re-prompt each session | Phone comms posture; Q37 |
...
**Won't prove:** latency, battery or thermals (S1, B3); that the event stream exists (B2); consent and authority safety (§8); speech accuracy (S3); that Owl's understanding routes interruptions and drops or resumes its speech correctly (S9); HTTPS and remote access (B1). It is thrown away, not shipped.
...
9. **Ignoring Owl's own voice,** so its speech does not pause itself (Q40 removes keyword detection; Q47 resumes speech only after a backchannel or noise).
10. **Silent mode:** whether a page can detect it, which Q14 and the Q39 acknowledgement sound depend on (Q37 settles the gesture rule for the briefing).
...
| Q14 | Sound | Subtle, on, volume, ducks, respects silent mode |
...
| Q24 | Long tasks | Ack < 1 s, milestones; interrupt pauses speech; "stop" stops task (refined by Q39, Q40, Q47) |
...
| Q31 | Cloning | None |
| Q32 | Transcripts | Auto-send, live; "no, I said…" corrects (refined by Q35, Q40) |
...
| Q39 | Slow hardware | Non-spoken ack within ~300 ms of detected end of turn on every tier (thinking mark + soft sound, which is feedback, not an alert); spoken "on it" as fast as possible, < 1 s on GPU/Mac |
| Q40 | Stop / steer / correct | No keyword matching; speaking pauses Owl; Owl's normal understanding decides (paused speech: Q47) |
...
| Q47 | Owl's speech when the owner talks | Pauses at once; a steer, stop or new question drops it; a backchannel or noise resumes it where it left off |
