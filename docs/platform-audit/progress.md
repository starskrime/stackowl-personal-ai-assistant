...
| 17 | Owl system (DNA, inner life, specialization) | 🔄 Tasks 1-11 shipped. Task 11: quality-weighted routing — `setQualityLookup()` on `SecretaryRouter`, EWMA reward proxy written per turn in `PostProcessor`, lookup wired in `core.ts` after lazy `SecretaryRouter` construction. 4970 tests passing. Head: `572943b`. Tasks 12-18 pending. | 2026-05-09 |
