...
| 18 | Providers (model routing, health, cost) | 🔄 Phase 3 (Winston architecture review) complete — 8 decisions locked (D1–D8): delete `engine/router.ts` + replace with IntelligenceRouter calls; make `intelligence` required in Gateway; add `ProviderCircuitBreaker` (CLOSED/OPEN/HALF-OPEN); add capability tags to `TierConfig`; wire CostTracker budget accessor into IntelligenceRouter; extend IntelligenceConfig with fallbacks/healthPolicy/costPolicy; delete `openai-compat.ts` (522 LOC dead); remove hardcoded model name defaults. Net file delta: −1. Phase 4 (brainstorming/spec) next. | 2026-05-10 |
