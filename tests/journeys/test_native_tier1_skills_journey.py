"""NATIVE TIER-1 SKILLS REACHABILITY JOURNEY — proves all 5 builtin skills load
...
# The 5 tier-1 skills this task introduces.
TIER1_SKILLS = [
    "verify-before-claim",
    "plan-and-track",
    "recover-and-retry",
...
@pytest.mark.parametrize("skill_name", TIER1_SKILLS)
...
def test_builtin_skill_body_has_required_sections(skill_name: str) -> None:
    """Every tier-1 skill body MUST carry the standard's required sections.
...
async def test_all_tier1_skills_reachable_through_assembly(
    tmp_db: DbPool, tmp_path: Path,
) -> None:
    """All 5 builtin tier-1 skills must be discoverable via the real assembly."""
