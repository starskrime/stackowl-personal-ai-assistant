"""NATIVE TIER-3 SKILLS REACHABILITY JOURNEY — proves all 3 builtin skills load
and are discoverable through the real loader + store.

For each of the 3 skills this test asserts:
...
# The 3 tier-3 skills this task introduces.
TIER3_SKILLS = [
    "code-exec-sandboxed",
    "media-generate",
    "channel-fallback",
...
# ---------------------------------------------------------------------------
# (b) Reachability: all 3 load through SkillsAssembly and are in the store
# ---------------------------------------------------------------------------

async def test_all_tier3_skills_reachable_through_assembly(
    tmp_db: DbPool, tmp_path: Path,
) -> None:
    """All 3 builtin tier-3 skills must be discoverable via the real assembly."""
    ...
    for skill_name in TIER3_SKILLS:
