...
DENY_SESSION must:
  ...
  (ii) be scoped per session_key — a different session still prompts;
...
async def test_deny_session_denies_current_call() -> None:
    ...
    result = await policy.request(tool_name="tool_x", channel="cli", session_key="s1")
...
async def test_deny_session_suppresses_subsequent_prompts_same_tool() -> None:
    ...
    first = await policy.request(tool_name="tool_x", channel="cli", session_key="s1")
    assert first is False
    assert prompter.call_count == 1  # prompted once

    second = await policy.request(tool_name="tool_x", channel="cli", session_key="s1")
...
async def test_deny_session_third_call_also_silent() -> None:
    ...
    await policy.request(tool_name="tool_x", channel="cli", session_key="s1")
    await policy.request(tool_name="tool_x", channel="cli", session_key="s1")
    result = await policy.request(tool_name="tool_x", channel="cli", session_key="s1")
    assert result is False
...
# ---------------------------------------------------------------------------
# (ii) DENY_SESSION is scoped per session_key
# ---------------------------------------------------------------------------
...
async def test_deny_session_does_not_affect_different_session() -> None:
    ...
    result_s1 = await policy.request(tool_name="tool_x", channel="cli", session_key="s1")
    assert result_s1 is False
    assert prompter.call_count == 1

    # s2 is a clean slate — must prompt normally
    result_s2 = await policy.request(tool_name="tool_x", channel="cli", session_key="s2")
...
async def test_deny_session_works_for_always_ask_tool() -> None:
    ...
    first = await policy.request(tool_name="execute_code", channel="cli", session_key="s1")
    assert first is False
    assert prompter.call_count == 1

    second = await policy.request(tool_name="execute_code", channel="cli", session_key="s1")
    assert second is False
...
async def test_session_approve_still_records_batch_grant() -> None:
    ...
    result_first = await policy.request(tool_name="tool_x", channel="cli", session_key="s1")
    assert result_first is True
    assert prompter.call_count == 1

    # Same tool — session batch should skip the prompt
    result_second = await policy.request(tool_name="tool_x", channel="cli", session_key="s1")
    assert result_second is True
    assert prompter.call_count == 1  # no new prompt

    # Different tool — must still prompt
    result_other = await policy.request(tool_name="tool_y", channel="cli", session_key="s1")
