"""Cross-channel identity: preferences scoped on identity_key, not session_key.
...
Invariants under test:
  ...
  3. FALLBACK byte-identical: identity_key="" falls back to session_key, preserving
     today's behavior for unconfigured channels.
  4. TIER set+get cross-channel: a tier written via one session_key+identity is
     readable via a DIFFERENT session_key sharing the same identity.
"""
...
async def test_cross_channel_pref_telegram_and_slack_see_same_identity(
    ...
    try:
        ...
        tg_state = _make_state(
            session_key="telegram:123", identity_key="owner-primary", channel="telegram"
        )
        tg_out = await classify.run(tg_state)

        # Slack session resolving to the same owner-primary
        sl_state = _make_state(
            session_key="slack:U0", identity_key="owner-primary", channel="slack"
        )
...
async def test_different_identities_are_isolated(tmp_db: DbPool) -> None:
    ...
    try:
        other_state = _make_state(
            session_key="telegram:999", identity_key="owner-other", channel="telegram"
        )
...
# ---------------------------------------------------------------------------
# Test 3: FALLBACK byte-identical — identity_key="" uses session_key
# ---------------------------------------------------------------------------
...
async def test_fallback_to_conversation_id_when_identity_key_empty(tmp_db: DbPool) -> None:
    ...
    try:
        state = _make_state(
            session_key="telegram:9", identity_key="", channel="telegram"
        )
    ...
    assert "language: Spanish" in ctx, (
        f"Fallback to session_key must surface session-keyed pref, got: {ctx[:200]}"
    )
...
def test_owner_key_for_state_uses_identity_when_set() -> None:
    ...
    state = _make_state(session_key="telegram:123", identity_key="owner-primary")
...
def test_owner_key_for_state_falls_back_to_session_key() -> None:
    """_owner_key_for_state returns session_key when identity_key is empty."""
    state = _make_state(session_key="telegram:123", identity_key="")
    key = _owner_key_for_state(state)
    assert key == "telegram:123", (
        f"Expected session_key fallback 'telegram:123', got: {key!r}"
    )
...
async def test_tier_cross_channel_same_identity_shares_tier(tmp_db: DbPool) -> None:
    ...
    # Read back via slack — different session_key, same identity_key
...
async def test_tier_different_identity_does_not_inherit(tmp_db: DbPool) -> None:
    ...
    other_state = _make_state(
        session_key="telegram:123", identity_key="owner-other", channel="telegram"
    )
    ...
    # Read via owner-other — same session_key but DIFFERENT identity_key
