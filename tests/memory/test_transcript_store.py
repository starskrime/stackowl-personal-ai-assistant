...
async def test_a_clean_turn_records_both_sides(store) -> None:
    ...
    written = await ts.record_turn(
        session_key=LANE, conversation_id=RUN, owl_name="Brain",
...
async def test_rows_are_stamped_with_the_owner(store) -> None:
    ...
    """
    ts, db = store
    await ts.record_turn(session_key=LANE, conversation_id=RUN, owl_name="Brain",
...
async def test_work_without_an_incarnation_is_not_a_conversation(store) -> None:
    ...
    ts, db = store
    assert await ts.record_turn(session_key="goal-x", conversation_id="", owl_name="Brain",
...
async def test_i6_a_rollover_never_touches_the_old_transcript(store) -> None:
    ...
    await ts.record_turn(session_key=LANE, conversation_id="20260726_040000_beef0000",
...
async def test_an_empty_user_utterance_records_nothing(store) -> None:
    ts, db = store
    assert await ts.record_turn(session_key=LANE, conversation_id=RUN, owl_name="Brain",
