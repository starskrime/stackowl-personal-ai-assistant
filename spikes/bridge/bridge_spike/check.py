...
async def run_check(install_name: str = DEFAULT_CHECK_INSTALL_NAME) -> CheckResult:
    """Start a real instance of the kit's server and prove the four things
    the story's AC requires of the automated check, then write
    ...
    return CheckResult(ok=ok, steps=steps, detail=detail, result_path=result_path)


def _write_result(payload: dict[str, object]) -> Path:
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    out_path = RESULTS_DIR / "B1-build-host-chromium.json"
