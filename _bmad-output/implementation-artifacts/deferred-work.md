...
- source_spec: `spec-4-3-a-declared-command-runs-through-one-door.md`
  summary: The AC's live check ("job pause and resume from the cronjob tool, driven through scripts/dev_ingress.py, runs as a COMMAND task, is audited with its actor, and appears in the journal") was not executed against the running platform.
  evidence: The running platform process (pid 2906, started 2026-09-18T18:29:04) predates every story-4.3 change by nearly a day and is running the pre-migration code — it has no COMMAND-task path to exercise, so driving `scripts/dev_ingress.py` against it right now would validate nothing about this story and would misrepresent verification if reported as such. The task's own standing constraint ("do not restart the live production core/gateway process yourself") makes a real restart-and-verify pass an operator action, not one available in this run. Strong indirect evidence exists instead: `tests/tools/test_cronjob_pause_resume_go_through_commands.py` drives `CronjobTool().execute(action="pause"/"resume", ...)` end-to-end against a real migrated SQLite `DbPool` and asserts a `kind='command'` task row lands, the job's `enabled` flag actually flips, and the tool's `ToolResult` shape is unchanged — the same behavior `dev_ingress.py` would trigger via a real Telegram-shaped message, just not through the literal running socket.
  location: `scripts/dev_ingress.py`; live process pid 2906
  severity: medium (unverified) — real risk if it were false is that the cronjob pilot migration behaves differently end-to-end than its in-process tests show (e.g. a wiring gap only the real orchestrator boot would surface); what would settle it: after the owner restarts core/gateway to pick up this story's code, run `uv run python scripts/dev_ingress.py "pause job <a real or throwaway job_id>"` and `"resume job <same job_id>"`, then confirm via `uv run stackowl db query` that a `kind='command'` row completed, `audit_log` shows `job_paused`/`job_resumed` with an actor, and `journal_events` shows `job.paused`/`job.resumed` + `command.enqueued`/`command.completed` rows for the same `command_id`.
  status: open
