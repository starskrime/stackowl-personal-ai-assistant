...
class ControlPlaneServer(SupervisedTask):
    ...
    def __init__(
        ...
        *,
        ...
        log.control_plane.info(
            ...
            extra={"_fields": {"has_health": health is not None}},
        )
    ...
    async def _handle_health(self, request: Any) -> Any:
        ...
        # ORDER IS BEHAVIOUR — origin before token. A browser request from
        # another origin may carry a valid credential, so checking the token
        # first would authenticate an attack before rejecting it.
        if not check_origin(
            ...
            log.control_plane.warning(
                "[control_plane] server.health: exit — refused, cross-origin",
        ...
        if principal is None:
            log.control_plane.warning(
                "[control_plane] server.health: exit — refused, unauthenticated"
            )
            return self._reject(web)

        if not principal.may(READ):
            log.control_plane.warning(
                "[control_plane] server.health: exit — refused, principal may "
                "not read",
                extra={"_fields": {"principal_id": principal.principal_id}},
            )
            return web.Response(status=403, text=UNAUTHORIZED_BODY)
