...
class ClarifyTool(Tool):
    ...
    def description(self) -> str:
        return (
            ...
            "LANE: resolve ambiguity / missing info by asking the human when the "
            "action it gates is irreversible, expensive, or DURABLE — anything "
            "that will keep running, or acting, after this turn ends (a scheduled "
            "agent, a recurring job, a standing rule). When the most likely action "
            "is one-off and cheap, do NOT clarify: act on the most likely "
            "interpretation and state your assumption. "
            "REVERSIBILITY IS NOT THE TEST. A thing you can undo later can still "
            "be wrong in a way only the user can settle, and asking costs one "
            "message where getting it wrong costs every run until they notice. "
