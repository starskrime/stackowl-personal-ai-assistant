-- Expand staged_facts.source_type CHECK to include 'agent_self' so that
-- facts authored by the agent's own self-mutation tool path are
-- distinguishable from human-authored ('manual') facts. This supports
-- recall down-ranking and privileged-context exclusion of self-authored
-- content (E4 design change #3 — provenance/privilege).
...
CREATE TABLE staged_facts_new (
    ...
    source_type         TEXT    NOT NULL
                            CHECK (source_type IN ('conversation', 'parliament', 'manual', 'webpage', 'screenshot', 'agent_self')),
