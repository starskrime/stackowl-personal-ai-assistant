...
async def record(conn: aiosqlite.Connection, event: JournalEvent) -> str:
    ...
    on an unregistered ``event.type`` or an ``attrs`` instance that fails its
    registered model. Redacts secret-shaped strings in ``attrs`` before the
    row is built. On ANY failure: 4-point-logs it, degrades
    ...
    try:
        ...
        await conn.execute(
            ...
            (
                ...
                event.attention, event.intensity, record_ref_json,
