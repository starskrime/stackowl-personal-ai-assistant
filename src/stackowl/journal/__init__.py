...
``link_events``, ``interaction_events`` and ``command_events`` for their side
effect (registering the task, job, heal, health, turn, memory, consent,
delivery/provider, channel, Needs-you item, budget-warning, gateway/core-link,
blocking-clarify and COMMAND-task-lifecycle event types), so any importer of
...
from stackowl.journal import command_events as _command_events  # noqa: F401 -- registration side effect
