...
# Channels on which the originating ``session_key`` IS the recipient ``chat_id``.
# For a Telegram PRIVATE chat the session_key (== str(user_id)) equals the chat_id
# (private chat_id == user_id), so a numeric session_key resolves directly to the
...
def resolve_target_chat_id(channel: str | None, session_key: str | None) -> int | None:
    ...
    ``session_key`` WHERE that is safe to do — i.e. only on channels where the
    session id is the chat id (Telegram private chats: session_key == str(user_id)
    == chat_id). It returns:

    * the numeric ``chat_id`` when ``channel`` is such a channel and ``session_key``
      is a clean integer (the common, correct case), or
    * ``None`` for any other channel, a missing/blank ``session_key``, or a
      non-numeric ``session_key`` (e.g. a Telegram group, whose chat_id != user_id)
    ...
    if not sid:
        log.notifications.warning(
            "[notifications] resolve_target_chat_id: no session_key on a "
