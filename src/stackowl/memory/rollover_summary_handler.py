...
class RolloverSummaryHandler(JobHandler):
    ...
    async def execute(self, job: Job) -> JobResult:
        ...
        # D09.2 — the corrected BEHAVIOUR, if the person corrected anything.
        # Runs on the SAME response as the summary, so it costs output tokens and
        # no extra call. Never allowed to affect the summary's outcome above.
        await self._record_correction(parse_correction(raw), ended=ended, job_id=job.job_id)
    ...
    async def _record_correction(
        self, correction: str | None, *, ended: str, job_id: str,
    ) -> bool:
        """Persist a corrected behaviour as a standing instruction for its owl.

        Returns True when something was written. NEVER RAISES: this runs after the
        summary has already been staged, and a failure here must not turn a
        successful rollover into a failed job.

        Written to the OWL, never to the global user profile. An instruction
        generated by a model and applied to every future conversation of every owl
        is too much reach for something nobody approved; scoped to the owl that
        was actually corrected, it is the same blast radius as the conversation it
        came from.
        """
        # 1. ENTRY / 2. DECISION — no correction is the normal case.
        if not correction:
            return False
        try:
            ...
            if not owl:
                log.memory.warning(
                    "[memory] rollover_summary.correction: no owl on the ended "
                    "conversation — NOT writing to the global profile instead",
                    extra={"_fields": {"job_id": job_id, "ended_conversation_id": ended}},
                )
                return False
            # 3. STEP — the ordinary curated write, with its capacity rules and
            #    decay. `until_changed`, not `permanent`: a working agreement is
            ...
            log.memory.info(
                "[memory] rollover_summary.correction: standing instruction recorded"
                if getattr(result, "ok", False)
                else "[memory] rollover_summary.correction: REFUSED by curated memory",
                extra={"_fields": {
                    "job_id": job_id, "owl": owl,
                    "ok": getattr(result, "ok", False),
                    "chars": len(correction),
                    "instruction": correction[:160],
                }},
            )
            return bool(getattr(result, "ok", False))
        except Exception as exc:  # never let a learning extra fail a rollover
            log.memory.error(
                "[memory] rollover_summary.correction: not recorded — the summary "
                "is unaffected",
                exc_info=exc,
                extra={"_fields": {"job_id": job_id, "ended_conversation_id": ended}},
            )
...
def _strip_fences(raw: str) -> str:
    """The fenced-JSON rule, in ONE place.

    Extracted when `parse_correction` arrived: two readers of the same response
    applying their own fence-stripping is the "two copies of one rule" shape, and
    they would drift the first time a model changed how it fences.
    """
    return _FENCE_CLOSE_RE.sub("", _FENCE_OPEN_RE.sub("", (raw or "").strip()))


def parse_correction(raw: str) -> str | None:
    """The corrected BEHAVIOUR from a rollover response, or None (D09.2).

    Returns an instruction to a future session — "answer directly; no preamble" —
    never a description of what went wrong. The operator's condition on adopting
    this at all (ESC-71) was that the platform's standing rule is to learn from
    WINS, never failures; storing the correction as a positive instruction is what
    reconciles a review prompt whose primary signal is user frustration with a
    learning loop that must not accumulate criticism of the user's own session.

    NONE IS THE NORMAL ANSWER. Most sessions contain no correction, and absent,
    empty and whitespace all mean the same thing. This never raises: it runs
    inside a background job whose real product is the summary, and a malformed
    optional field may not cost that.
    """
