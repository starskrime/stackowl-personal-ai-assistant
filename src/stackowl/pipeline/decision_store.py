...
One row per session (UPSERT on ``session_key``) — only the latest turn is kept,
...
_UPSERT = (
    "INSERT INTO turn_decisions (session_key, trace_id, created_at, decisions_json) "
    "VALUES (?, ?, ?, ?) "
...
class TurnDecisionStore:
    ...
    async def save(
        ...
        *,
        ...
        """UPSERT this turn's decisions for ``session_key``. Best-effort, never raises."""
        # 1. ENTRY
        log.engine.debug(
            "[decision_store] save: entry",
            extra={"_fields": {"session_key": session_key, "count": len(decisions)}},
        )
        try:
            ...
            await self._db.execute(
                _UPSERT, (session_key, trace_id, time.time(), payload)
            )
            # 4. EXIT
            log.engine.debug(
                "[decision_store] save: exit — persisted",
                extra={"_fields": {"session_key": session_key, "json_len": len(payload)}},
            )
    ...
    async def latest(self, session_key: str) -> tuple[Decision, ...] | None:
        """Read the latest persisted decisions for ``session_key``. Never raises.
        ...
        try:
            rows = await self._db.fetch_all(
                "SELECT decisions_json FROM turn_decisions WHERE session_key = ?",
                (session_key,),
            )
            if not rows:
                # 4. EXIT — no snapshot
