...
## When to Use
When a message or file must reach the user but the primary channel send attempt fails or is unavailable. Ensures delivery is attempted on alternate reachable channels before reporting the outcome, and is honest if all channels fail.
...
## Prerequisites
- `send_message` and `send_file` for delivery attempts.
- A known alternate channel or recipient for the same user.
- The primary send must have been ATTEMPTED before any fallback.

## How to Run
1. Attempt delivery on the primary channel.
2. On failure, identify an alternate reachable channel or recipient.
3. Attempt delivery there.
4. Record which channel actually succeeded.
5. If every channel fails, say so plainly.

## Quick Reference
- Never report "sent" for a send that failed — that is an honesty violation.
- Fallback happens only AFTER a real primary attempt, never instead of one.
- The reply names the channel that actually carried the message.
- All channels failing is a reportable outcome, not something to hide.
