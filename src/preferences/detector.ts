...
export class PreferenceDetector {
  ...
  async detect(
    ...
    const allAlreadySet = knownKeys.every(
      (k) =>
        store.get(k, channel) !== undefined ||
        store.get(k, "all") !== undefined,
    );
    const isShortRepeat =
      allAlreadySet && userMessage.trim().split(/\s+/).length < 12;
    if (isShortRepeat) {
      log.engine.info(
        "[Preferences] All known preferences already set — skipping LLM extraction",
      );
      return [];
    }
    ...
    try {
      ...
      for (const p of extracted) {
        ...
        updated.push(p.key);
        log.engine.info(
          `[Preferences] Stored: ${p.key} = ${JSON.stringify(p.value)} (channel: ${p.channel})`,
        );
      }
      return updated;
    } catch (err) {
      log.engine.warn(
        `[Preferences] Detection failed: ${err instanceof Error ? err.message : String(err)}`,
      );
      return [];
    }
  }
