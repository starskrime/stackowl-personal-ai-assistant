...
for (let i = 0; i < 26; i++) {
  ...
  const codes = [
    0, 11, 8, 2, 14, 3, 5, 4, 34, 38, 40, 37, 46, 45, 31, 35, 12, 15, 1, 17, 32,
    9, 13, 7, 16, 6,
  ];
...
export async function pressKey(
  ...
  if (keyCode === undefined) {
    ...
    throw new Error(
      `Unknown key: "${key}". Use key names like: enter, tab, escape, up, down, f1, a, b, etc.`,
    );
  }
...
// ─── Terminal-style Render (for debugging/visualization) ─────────
...
export function renderTerminalGrid(
  ...
  for (const el of elements) {
    const x = Math.min(Math.floor(el.position.x), width - 1);
    const y = Math.min(Math.floor(el.position.y), height - 1);
    const w = Math.min(el.size.width, width - x);
    const h = Math.min(el.size.height, height - y);
...
export function elementToBoxString(el: ElementBox): string {
  const pos = el.position;
  const size = el.size;

  let content = `Element: ${el.element || "unknown"}`;
  if (el.role) content += ` (role: ${el.role})`;
  if (el.text) content += ` — "${truncate(el.text, 50)}"`;

  return (
    `┌${"-".repeat(78)}┐\n` +
    `│ ${content.padEnd(76)} │\n` +
    `├${"-".repeat(78)}┤\n` +
    `│ Position: x=${pos.x.toString().padStart(4)}, y=${pos.y
      .toString()
      .padStart(4)}                      │\n` +
    `│ Size: ${size.width.toString().padStart(4)}x${size.height
      .toString()
      .padStart(4)}                               │\n` +
    `└${"-".repeat(78)}┘`
  );
...
// ─── Structured Analysis to Text ──────────────────────────────────

export function analyzeScreenshotToText(analysis: ScreenshotAnalysis): string {
  const lines: string[] = [];

  lines.push(`SCREEN ANALYSIS`);
  lines.push(`==============`);
  lines.push(
    `Resolution: ${analysis.screen.resolution.width}x${analysis.screen.resolution.height}`,
  );
  lines.push(`Scale factor: ${analysis.screen.scale}x`);
  lines.push(`Elements found: ${analysis.elements.length}`);
  lines.push(`Timestamp: ${new Date(analysis.timestamp).toISOString()}`);
  lines.push("");

  if (analysis.elements.length === 0) {
    lines.push("No UI elements detected.");
  } else {
    lines.push(`UI ELEMENTS (${analysis.elements.length})`);
    lines.push(`---------------------`);
    ...
    for (let i = 0; i < analysis.elements.length; i++) {
      ...
      lines.push(`\n[${i + 1}] ${el.role || "element"}`);
      if (el.text) {
        lines.push(`    Text: "${truncate(el.text, 100)}"`);
      }
      lines.push(
        `    Position: (${analysis.elements[i].position.x}, ${analysis.elements[i].position.y})`,
      );
      lines.push(
        `    Size: ${analysis.elements[i].size.width}x${analysis.elements[i].size.height}`,
      );
    }
  }
...
// ─── Image File to Text Conversion (for screenshots) ─────────────
...
export function formatScreenshotForLLM(
  ...
  if (analysis) {
    content += `\n${analyzeScreenshotToText(analysis)}\n`;
  } else {
    content += `\n(Element analysis not available. Use analyze_screen action in computer_use tool for UI details.)\n`;
  }
