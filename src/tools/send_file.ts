...
async function downloadToLocal(url: string, cwd: string): Promise<string> {
  ...
  if (contentType.includes("text/html") && !contentType.includes("image")) {
    throw new Error(
      `URL returned HTML instead of an image. The link may be a web page, not a direct file.`,
    );
  }
    ...
    parameters: {
      ...
      properties: {
        path: {
          type: "string",
          description:
            "Absolute path, workspace-relative path, or HTTP/HTTPS URL to the file to send",
        },
        caption: {
          type: "string",
