    ...
    parameters: {
      ...
      properties: {
        action: {
          type: "string",
          description:
            'Action: "generate" (create new), "list" (all journals), "view" (specific), "search" (by keyword)',
        },
        period: {
          type: "string",
    ...
    try {
      switch (action) {
        case "generate": {
          ...
          return (
            `**${period.charAt(0).toUpperCase() + period.slice(1)} Growth Journal**\n\n${entry.narrative}\n\n` +
            `---\n` +
        ...
        case "list": {
          ...
          if (entries.length === 0)
            return 'No journal entries yet. Use action "generate" to create one.';
          return (
            "**Growth Journals:**\n" +
            entries
              .map(
                (e) =>
                  `- **${e.id}** (${e.period}) — ${new Date(e.generatedAt).toLocaleDateString()}`,
              )
              .join("\n")
          );
        }
        ...
        case "search": {
          ...
          return (
            ...
            results
              .map((e) => `- **${e.id}**: ${e.narrative.slice(0, 100)}...`)
              .join("\n")
          );
        }
