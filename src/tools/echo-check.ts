  ...
  definition = {
    ...
    parameters: {
      ...
      properties: {
        action: {
          type: 'string',
          description: 'Action: "analyze" (full analysis), "challenge" (generate a challenge), "status" (last analysis)',
        },
        intensity: {
          type: 'string',
          description: 'Challenge intensity: "gentle", "balanced" (default), or "relentless"',
        },
      },
      required: ['action'],
    },
    ...
    try {
      switch (action) {
        ...
        case 'challenge': {
          ...
          return challenge || 'No bias patterns detected to challenge. Your thinking appears balanced.';
        }

        case 'status': {
          const last = detector.getLastAnalysis();
          if (!last) return 'No analysis has been run yet. Use action "analyze" first.';
