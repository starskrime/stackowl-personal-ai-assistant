...
export class ToolOutcomeStore {
  ...
  async record(
    ...
    if (this.index.outcomes.length > ToolOutcomeStore.MAX_OUTCOMES) {
      this.index.outcomes = this.index.outcomes.slice(-ToolOutcomeStore.MAX_OUTCOMES);
    }
  ...
  getBestPattern(userMessage: string): ToolPattern | null {
    ...
    if (pattern.observations < ToolOutcomeStore.MIN_OBSERVATIONS_FOR_PATTERN) return null;
  ...
  getTopPatterns(limit: number = 5): ToolPattern[] {
    return Object.values(this.index.patterns)
      .filter(p => p.observations >= ToolOutcomeStore.MIN_OBSERVATIONS_FOR_PATTERN)
      .sort((a, b) => {
  ...
  private rebuildPattern(requestType: string): void {
    const relevant = this.index.outcomes.filter(o => o.requestType === requestType);
    if (relevant.length === 0) return;

    // Find the most successful tool combination
    const comboCounts = new Map<string, { success: number; total: number; satisfaction: number }>();

    for (const outcome of relevant) {
      const key = outcome.toolsUsed.sort().join('+');
      const existing = comboCounts.get(key) ?? { success: 0, total: 0, satisfaction: 0 };
      existing.total++;
  ...
  private async save(): Promise<void> {
    ...
    try {
      await writeFile(this.filePath, JSON.stringify(this.index, null, 2), 'utf-8');
      this.dirty = false;
    } catch (err) {
