...
export const PatchTool: ToolImplementation = {
    definition: {
        name: 'patch_tool',
        description: 'Rewrite the source code of a broken or buggy tool. Use this when you are stuck in an error loop and realize a tool implementation itself is flawed. Core tools require a restart, but synthesized tools will be hot-reloaded automatically.',
        parameters: {
            type: 'object',
            properties: {
                toolName: {
                    type: 'string',
                    description: 'The exact name of the tool to patch (e.g. "shell", "read_file", or a synthesized tool name).',
                },
                newSourceCode: {
                    type: 'string',
                    description: 'The COMPLETE, fully re-written TypeScript source code for the tool. This MUST be the entire file content, not a git-diff. Ensure the code returns a default export or matches the ToolImplementation structure.',
                },
                description: {
                    type: 'string',
                    description: 'A brief description of what bug you are fixing inside this tool.',
                }
            },
    ...
    async execute(args: Record<string, unknown>, _context: ToolContext): Promise<string> {
        ...
        log.tool.warn(`[Toolsmith] The AI is patching tool '${toolName}'. Reason: ${description}`);

        // Try to find the tool file
        // 1. Check synthesized directory first
        const synthesizedPath = join(SYNTHESIZED_DIR, `${toolName}.ts`);
        const builtInPath = resolve(process.cwd(), 'src', 'tools', `${toolName}.ts`);
