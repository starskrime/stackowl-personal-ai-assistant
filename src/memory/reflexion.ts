...
export type MemoryCategory = 'preference' | 'fact' | 'decision' | 'project' | 'context';
...
function categorizeFact(content: string): MemoryCategory {
  ...
  if (/prefer|like|hate|want|dislike|favorite|better|instead/i.test(lower)) return 'preference';
  if (/decided|going to|will use|chose|choosing|picking/i.test(lower)) return 'decision';
  if (/project|repo|codebase|building|working on|implementing/i.test(lower)) return 'project';
  if (/remember|note|important|fact|truth|always|never/i.test(lower)) return 'fact';
  return 'context';
}

function extractTopics(content: string): string[] {
  const techTerms = [
    'typescript', 'javascript', 'python', 'rust', 'golang', 'java', 'swift',
    'react', 'vue', 'angular', 'nextjs', 'node', 'deno',
    'docker', 'kubernetes', 'k8s', 'aws', 'gcp', 'azure',
    'postgresql', 'mysql', 'mongodb', 'redis',
    'openai', 'anthropic', 'claude', 'ollama', 'llm',
    'git', 'github', 'gitlab',
...
export class MemoryReflexionEngine {
  ...
  constructor(workspacePath: string, provider?: ModelProvider, owl?: OwlInstance) {
    ...
    this.store = {
      entries: {},
      byCategory: { preference: [], fact: [], decision: [], project: [], context: [] },
      byTopic: {},
      lastReflexed: '',
      lastConsolidated: '',
      totalEntries: 0,
  ...
  async init(): Promise<void> {
    if (!existsSync(this.memoryDir)) await mkdir(this.memoryDir, { recursive: true });
    if (!existsSync(this.archiveDir)) await mkdir(this.archiveDir, { recursive: true });
    await this.load();
  }
  ...
  private async save(): Promise<void> {
    ...
    await writeFile(this.indexPath, JSON.stringify(this.store, null, 2), 'utf-8');
  }
  
  async consolidate(messages: ChatMessage[], sessionId?: string): Promise<ConsolidationResult> {
    await this.init();
    ...
    const transcript = relevant.slice(-20).map(m => `[${m.role.toUpperCase()}]: ${(m.content ?? '').slice(0, 300)}`).join('\n\n');
    
    const result: ConsolidationResult = { entriesAdded: 0, entriesUpdated: 0, duplicatesSkipped: 0 };
      ...
      try {
        ...
        for (const fact of facts) {
          ...
          if (existing) {
            ...
            const entry = this.createEntry(fact.content, fact.category, sessionId);
            this.addEntry(entry);
            result.entriesAdded++;
          }
        }
    ...
    await this.save();
    if (this.store.totalEntries > COMPRESSION_THRESHOLD) await this.maybeCompress();
    return result;
  }
  
  private async extractFactsWithLLM(transcript: string): Promise<{ content: string; category: MemoryCategory }[]> {
    ...
    let jsonStr = response.content.trim().replace(/^```json?\s*/i, '').replace(/\s*```$/i, '').trim();
    const parsed = JSON.parse(jsonStr);
    if (!Array.isArray(parsed)) return [];
    
    return parsed.slice(0, 8).map((item: any) => ({
      content: String(item.content ?? '').slice(0, 500),
      category: (['preference', 'fact', 'decision', 'project', 'context'].includes(item.category))
  ...
  private createEntry(content: string, category: MemoryCategory, sessionId?: string): MemoryEntry {
    return {
      id: generateId(),
      content,
      category,
  ...
  private addEntry(entry: MemoryEntry): void {
    ...
    if (!this.store.byCategory[entry.category]) this.store.byCategory[entry.category] = [];
  ...
  private findSimilar(content: string): MemoryEntry | undefined {
    ...
    for (const entry of Object.values(this.store.entries)) {
      ...
      const entryWords = new Set(entryNormalized.split(' ').filter(w => w.length > 3));
      const searchWords = new Set(normalized.split(' ').filter(w => w.length > 3));
      const intersection = [...entryWords].filter(w => searchWords.has(w));
      const similarity = intersection.length / Math.max(entryWords.size, searchWords.size);
  ...
  private async compressOldEntries(): Promise<string[]> {
    ...
    for (const entry of entries.filter(e => e.accessCount < 3 && e.importance < 0.5).slice(0, ARCHIVE_BATCH_SIZE)) {
  ...
  private async generateInsights(): Promise<string[]> {
    ...
    const recentEntries = Object.values(this.store.entries).slice(0, 30).map(e => `[${e.category}]: ${e.content}`).join('\n');
    
    try {
      const response = await this.provider.chat([
        { role: 'system', content: 'You are a memory analyst. Output only valid JSON.' },
        { role: 'user', content: `Analyze these memories and identify patterns:\n\n${recentEntries}\n\nReturn 2-3 short insights as a JSON array.` },
      ]);
      let jsonStr = response.content.trim().replace(/^```json?\s*/i, '').replace(/\s*```$/i, '').trim();
      return JSON.parse(jsonStr) || [];
    } catch (err) {
      log.engine.warn(`[Reflexion] Insight generation failed: ${err instanceof Error ? err.message : err}`);
      return [];
    }
  }
  ...
  private async reviveForgotten(): Promise<string[]> {
    ...
    for (const entry of Object.values(this.store.entries)) {
      ...
      if (daysSinceAccess > REVIVAL_CHECK_INTERVAL_DAYS && entry.importance > 0.3) {
        entry.accessCount++;
  ...
  private calculateHealth(): number {
    ...
    let health = 100;
    if (entries.length > MAX_ENTRIES) health -= Math.min(30, (entries.length - MAX_ENTRIES) * 0.5);
    const daysSinceReflex = (Date.now() - new Date(this.store.lastReflexed || 0).getTime()) / (1000 * 60 * 60 * 24);
  ...
  async retrieve(query: string, maxEntries: number = 10): Promise<MemoryEntry[]> {
    await this.init();
    ...
    const scored = Object.values(this.store.entries).map(entry => {
      ...
      for (const word of queryWords) { if (contentLower.includes(word)) score += 5; }
      if (entry.category === 'preference') score += 3;
      if (entry.category === 'decision') score += 2;
      return { entry, score };
    });
    
    const results = scored.sort((a, b) => b.score - a.score).slice(0, maxEntries).map(s => {
      s.entry.accessCount++;
    ...
    this.save().catch((err) => {
      log.engine.warn(`[Reflexion] Save failed after retrieval: ${err instanceof Error ? err.message : err}`);
    });
    return results;
  }
  ...
  async getForSystemPrompt(maxChars: number = 4000): Promise<string> {
    await this.init();
    const entries = Object.values(this.store.entries).filter(e => !e.compressed).sort((a, b) => b.importance - a.importance);
    const lines: string[] = ['## Known Facts & Preferences'];
    const categories: MemoryCategory[] = ['preference', 'decision', 'project', 'fact', 'context'];
    ...
    for (const cat of categories) {
      ...
      for (const entry of catEntries.slice(0, 10)) {
        const importance = entry.importance > 0.7 ? '🔴' : entry.importance > 0.4 ? '🟡' : '⚪';
        lines.push(`${importance} ${entry.content}`);
      }
    }
    
    const full = lines.join('\n');
    return full.length > maxChars ? full.slice(0, maxChars) + '\n\n[...more memories truncated]' : full;
  }
  
  getStats(): { total: number; byCategory: Record<string, number>; health: number; lastReflex: string } {
    const byCategory: Record<string, number> = {};
    for (const [cat, ids] of Object.entries(this.store.byCategory)) byCategory[cat] = ids.length;
    return { total: this.store.totalEntries, byCategory, health: this.calculateHealth(), lastReflex: this.store.lastReflexed || 'never' };
  }
  ...
  async delete(id: string): Promise<void> {
    ...
    this.store.byCategory[entry.category] = this.store.byCategory[entry.category].filter(i => i !== id);
    await this.save();
  }
