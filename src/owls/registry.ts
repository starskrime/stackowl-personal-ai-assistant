...
import type { OwlPersona, OwlDNA, OwlInstance, ChallengeLevel } from './persona.js';
    ...
    private async loadFromDirectory(dirPath: string): Promise<void> {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                console.error(`[OwlRegistry] Failed to load owl from ${owlMdPath}: ${msg}`);
            }
        }
    }
    ...
    private async loadOrCreateDNA(persona: OwlPersona): Promise<OwlDNA> {
        ...
        if (existsSync(dnaPath)) {
            try {
                ...
                console.warn(`[OwlRegistry] Corrupt DNA for ${persona.name}, creating fresh.`);
            }
        }
