...
export class CollabSessionManager {
  ...
  createSession(
    ...
    log.info(`Created collab session "${name}" (${session.id}) by ${owner.displayName}`);
    return session;
  }
  ...
  joinSession(
    ...
    if (session.participants.length >= session.settings.maxParticipants) {
      throw new Error(`Session is full (max ${session.settings.maxParticipants})`);
    }
  ...
  addMessage(
    ...
    if (session.messages.length >= this.config.maxMessagesPerSession) {
      throw new Error(`Session message limit (${this.config.maxMessagesPerSession}) reached`);
    }
    ...
    const participant = session.participants.find(p => p.userId === message.userId);
  ...
  buildCollabContext(sessionId: string): string {
    ...
    const participantCount = session.participants.filter(p => p.role !== 'observer').length;
    const lines: string[] = [
    ...
    for (const p of session.participants) {
      const expertise = p.expertise?.length ? ` expertise="${p.expertise.join(', ')}"` : '';
      const suffix = p.role === 'observer' ? ' (observing)' : '';
      lines.push(`    <user id="${p.userId}" role="${p.role}"${expertise}>${p.displayName}${suffix}</user>`);
    }

    lines.push('  </participants>');
    lines.push('  <instructions>');
    lines.push('    This is a multi-user collaborative session. Address participants by name.');
