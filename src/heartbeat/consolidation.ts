...
export class MemoryConsolidator {
  ...
  async consolidateSession(userId: string): Promise<void> {
    console.log(
      `[Consolidator] 🧠 Starting memory consolidation for user: ${userId}`,
    );
    ...
    if (!existsSync(sessionPath)) {
      console.log(
        `[Consolidator] No session found for user: ${userId}. Skipping.`,
      );
      return;
    }
    ...
    try {
      ...
      const parsed = JSON.parse(factsJson) as {
        extractedFacts: { fact: string; weight: number }[];
      };

      if (parsed.extractedFacts && parsed.extractedFacts.length > 0) {
        console.log(
          `[Consolidator] 🧩 Found ${parsed.extractedFacts.length} new facts. updating DNA...`,
        );
  ...
  private async updateDNA(
    newFacts: { fact: string; weight: number }[],
  ): Promise<void> {
    // Read the current DNA from disk
    const dnaPath = join(
      this.owl.persona.sourcePath.replace("OWL.md", ""),
      "owl_dna.json",
    );
    ...
    try {
      ...
      for (const f of newFacts) {
        ...
        if (dnaData.learnedPreferences[f.fact]) {
          dnaData.learnedPreferences[f.fact] =
            (dnaData.learnedPreferences[f.fact] + f.weight) / 2;
        } else {
