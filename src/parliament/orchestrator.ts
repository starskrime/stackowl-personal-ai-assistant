...
import type { ParliamentConfig, ParliamentSession, OwlPosition, ParliamentCallbacks } from './protocol.js';
...
export class ParliamentOrchestrator {
    ...
    async convene(config: ParliamentConfig): Promise<ParliamentSession> {
        ...
        try {
            ...
            try {
                ...
                log.engine.error(`[Parliament] Failed to generate pellet: ${pelletError}`);
            }
    ...
    private async runRound1(
        ...
        for (const owl of session.config.participants) {
            ...
            let prompt = `PARLIAMENT TOPIC: ${session.config.topic}\n\n` +
    ...
    private async runRound2(
        ...
        if (!challenger) {
            challenger = session.config.participants
        ...
        for (const p of session.config.participants) {
            if (p.persona.name !== challenger.persona.name && response.content.includes(p.persona.name)) {
        ...
        if (!targetOwl) {
            for (const [owlName, persp] of perspectives) {
                if (owlName !== challenger.persona.name && response.content.includes(persp.label)) {
                    targetOwl = owlName;
                    break;
                }
            }
    ...
    private async runRound3(
        ...
        if (!synthesizer) synthesizer = session.config.participants.find(o => o.persona.name === 'Noctua');
        ...
        const positionsText = session.positions.map(p => {
            const persp = perspectives.get(p.owlName);
            const label = persp ? `${persp.label} (${p.owlName})` : p.owlName;
            return `- ${label} [${p.position}]: ${p.argument}`;
        }).join('\n');

        const challengesText = session.challenges.map(c => {
    ...
    formatSessionMarkdown(
        ...
        if (session.challenges.length > 0) {
            ...
            for (const c of session.challenges) {
                const persp = perspectives?.get(c.owlName);
                const label = persp ? `${persp.emoji} ${persp.label}` : `**${c.owlName}**`;
