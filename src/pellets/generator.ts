...
export class PelletGenerator {
    ...
    async generate(
        ...
        const prompt = `You are digesting a conversation or research output to create a "Pellet" — a compressed, highly structured knowledge artifact.\n\n` +
            `Source Material:\n${sourceMaterial}\n\n` +
        ...
        try {
            ...
            console.error('[PelletGenerator] Failed to parse LLM output as JSON. Falling back to dumb string logic.');
        ...
        return {
            ...
            source: sourceName,
            owls: Array.isArray(parsed.owlsInvolved) ? parsed.owlsInvolved : [owl.persona.name],
