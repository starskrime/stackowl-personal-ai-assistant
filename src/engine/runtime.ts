...
export class OwlEngine {
  ...
  async run(
    ...
    let routeDecision = await ModelRouter.route(userMessage, provider, config);
    ...
    if (tools && tools.length > 0) {
      ...
      while (
          ...
          if (toolRegistry && !toolRegistry.has(toolCall.name)) {
            ...
            if (context.capabilityLedger) {
              context.capabilityLedger
                .recordUsage(toolCall.name, success)
                .catch(() => { });
            }
            ...
            if (!success) {
              ...
              const analysisPrompt =
                `[SYSTEM OVERRIDE: ERROR ANALYSIS REQUIRED!]\n` +
                `The tool "${toolCall.name}" just threw an error (failure #${streak}).\n` +
                `Error Details: \n${toolResult}\n\n` +
                `You must STEP BACK and reason through this failure before acting:\n` +
                `1. Evaluate the Error: Is this a missing prerequisite (like pulling a Docker image or running 'npm install'), bad arguments, OR a bug in the tool itself?\n` +
                `2. If a prerequisite is missing, use an existing tool like 'run_shell_command' to fix the environment.\n` +
                `3. If the tool implementation is genuinely broken or buggy, YOU MUST USE THE 'patch_tool' TO REWRITE ITS SOURCE CODE.\n\n` +
                (streak >= MAX_TOOL_FAIL_STREAK
                  ? `🛑 CRITICAL: You are trapped in an infinite loop. This tool has failed ${streak} times. DO NOT call "${toolCall.name}" again. You MUST either rewrite it using 'patch_tool' or execute a radically different strategy.`
                  : `Think carefully before your next tool call.`);

              log.engine.warn(
                `Tool "${toolCall.name}" failed (streak: ${streak}) — injecting global self-healing heuristic`,
              );
              ...
              if (streak >= MAX_TOOL_FAIL_STREAK + 1) {
                ...
                log.engine.warn(
                  `LLM ignored error analysis ${streak}x — breaking ReAct loop`,
                );
        ...
        if (globalConsecutiveFailures >= 2) {
          const newRoute = await ModelRouter.route(userMessage, currentProvider, config, globalConsecutiveFailures);
  ...
  private async buildSystemPrompt(
    ...
    // Inject available skills (from ClawHub-compatible skill system)
    ...
    prompt +=
      "You have a set of tools available. If a user asks you to do something that requires a tool or capability you do NOT have,\n";
    prompt +=
      "you MUST include this exact marker somewhere in your response:\n";
    prompt +=
      "[CAPABILITY_GAP: one sentence describing what you tried to do and why you cannot]\n";
    prompt +=
      "Example: [CAPABILITY_GAP: tried to take a screenshot but no screen capture tool is available]\n";
    prompt +=
      "Example: [CAPABILITY_GAP: tried to send an email but no email/SMTP tool is available]\n";
    prompt +=
      "This marker is invisible to the user — it is stripped before display. Do NOT add it if you simply choose not to do something for ethical reasons.\n";

    // Core directive: challenge mode
    prompt += "\n## IMPORTANT: Challenge Mode\n";
    prompt +=
      "You are NOT a yes-man. You are a colleague who genuinely cares about getting the right answer.\n";
    ...
    prompt +=
      "- Always be respectful but honest. Disagree with ideas, not people.\n";
  ...
  private shouldChallenge(_userMessage: string, owl: OwlInstance): boolean {
    ...
    const threshold =
      challengeLevels[owl.dna.evolvedTraits.challengeLevel] ?? 0.4;
