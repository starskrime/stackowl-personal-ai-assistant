...
export class FilePerch implements PerchPoint {
    ...
    private shouldProcess(filename: string): boolean {
        ...
        const ALLOWED_EXTS = new Set(['.ts', '.tsx', '.js', '.jsx', '.json', '.md', '.yaml', '.yml', '.toml', '.css', '.html', '.py', '.sh']);
        if (!ALLOWED_EXTS.has(extname(filename))) return false;

        if (filename.includes('sessions/') || filename.includes('sessions\\')) return false;
    ...
    private handleFileChange(_eventType: string, filename: string) {
        ...
        if (!existsSync(fullPath)) {
            // File was deleted
            if (prevSnapshot) {
                this.pendingChanges.set(filename, { type: 'deleted', linesRemoved: prevSnapshot.lineCount });
            ...
            try {
                ...
                if (!prevSnapshot) {
                    ...
                    this.pendingChanges.set(filename, { type: 'modified', linesAdded, linesRemoved });
                }
    ...
    private flushPendingChanges(): void {
        ...
        const diffSummary = (totalLinesAdded > 0 || totalLinesRemoved > 0)
        ...
        if (this.emitFn) {
            this.emitFn({
                type: 'file_change',
                source: totalFiles === 1 ? (created[0] || modified[0] || deleted[0]) : `${totalFiles} files`,
                details,
            });
        }
    }
