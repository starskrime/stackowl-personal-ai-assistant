...
export class KnowledgeResearcher {
  ...
  async research(domain: string, context?: string): Promise<ResearchResult> {
    ...
    if (existing.length >= MAX_PELLETS) {
      ...
      for (const pellet of toEvict) {
        ...
        log.evolution.debug(
          `  evicted: "${pellet.title}" (${pellet.generatedAt})`,
        );
      }
    }
    ...
    for (const question of questions.slice(0, 2)) {
      try {
        ...
        log.evolution.warn(
          `  ✗ Question failed: ${err instanceof Error ? err.message : err}`,
        );
      }
    }
      ...
      const response = await this.provider.chat([
        {
          role: "system",
          content:
            "You are a research question generator. Output only valid JSON arrays.",
        },
  ...
  private async answerAndStore(
    ...
    const relatedMatch = response.content.match(
      /RELATED_JSON:\s*(\[[\s\S]*?\])/,
    );
    if (relatedMatch) {
      try {
    ...
    const slug =
      `learn-${domain
        ...
        .replace(/\s+/g, "-")
        .replace(/[^a-z0-9-]/g, "")
        .slice(0, 30)}` + `-${uuidv4().substring(0, 6)}`;
