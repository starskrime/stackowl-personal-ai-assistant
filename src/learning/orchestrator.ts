...
import type { ReflexionResult, ConsolidationResult } from '../memory/reflexion.js';
...
export class LearningOrchestrator {
  ...
  constructor(
    ...
    this.synthesizer = new KnowledgeSynthesizer(provider, owl, config, pelletStore, workspacePath);
    this.reflexionEngine = new MemoryReflexionEngine(workspacePath, provider, owl);
    if (providerRegistry) {
      this.selfHealer = new SelfHealer(providerRegistry, join(workspacePath, '..'), workspacePath);
    }
  }
  ...
  async processConversation(messages: ChatMessage[]): Promise<LearningCycle> {
    ...
    try {
      ...
      cycle.insightsExtracted = insights.topics.length + insights.knowledgeGaps.length;
      ...
      const criticalTopics = fusion.fusedTopics.filter(
        (t: FusedTopic) => t.urgency >= 60 || t.priorityOverride === 'critical'
      );

      if (criticalTopics.length > 0 && this.synthesizer) {
        const report = await this.synthesizer.synthesize(criticalTopics);
  ...
  async learnTopic(topic: string, deepResearch: boolean = true): Promise<LearningCycle> {
  ...
  private getDaysSinceLastReflexion(): number {
    ...
    return (Date.now() - new Date(lastReflex).getTime()) / (1000 * 60 * 60 * 24);
  }
}
