...
function classifyError(err: unknown): ErrorClass {
    ...
    if (msg.includes('rate') || msg.includes('429') || msg.includes('quota') || msg.includes('too many')) return 'rate_limit';
...
export class LearningEngine {
    ...
    constructor(
        ...
        if (providerRegistry) {
            ...
            this.selfHealer = new SelfHealer(providerRegistry, projectRoot, workspacePath);
        }
    }
    ...
    async processConversation(messages: ChatMessage[]): Promise<void> {
        ...
        if (this.health.consecutiveFailures > 0) {
            const backoff = Math.min(
                LearningEngine.BACKOFF_BASE_MS * Math.pow(2, this.health.consecutiveFailures - 1),
                30_000, // cap at 30s
            );
            log.evolution.info(
        ...
        if (this.health.consecutiveFailures >= LearningEngine.MAX_CONSECUTIVE_FAILURES) {
    ...
    private async processConversationDegraded(
        ...
        if (this.health.totalAttempts % 5 === 0) {
            log.evolution.info('[Learning] Degraded mode: attempting recovery probe...');
            try {
                ...
                if (probe.topics.length > 0 || probe.domains.length > 0) {
                    log.evolution.evolve('[Learning] Recovery probe succeeded! Exiting degraded mode.');
                ...
                log.evolution.info('[Learning] Recovery probe failed, staying in degraded mode.');
            }
        }
        ...
        try {
            const result = await this.selfHealer.heal({
                ...
                context:
                    ...
                    `Recent user message: ${userMessages.slice(-1).map(m => (m.content ?? '').slice(0, 200)).join('')}`,
            });
    ...
    private extractTopicsHeuristic(userMessages: ChatMessage[]): string[] {
        ...
        for (const msg of userMessages) {
            ...
            const howTo = text.match(/how (?:to|do|can|should|does)\s+(.{5,40}?)(?:\?|$)/gi);
            if (howTo) {
                for (const match of howTo) {
                    const topic = match.replace(/^how (?:to|do|can|should|does)\s+/i, '').replace(/\?$/, '').trim();
        ...
        for (const topic of queue) {
            try {
                ...
                if (errClass === 'rate_limit') {
                    log.evolution.warn('[Study] Rate limited — stopping study session early.');
                    break;
                }
            }
        }
    ...
    async getSummaryLine(): Promise<string> {
        ...
        const healthTag = this.health.consecutiveFailures >= LearningEngine.MAX_CONSECUTIVE_FAILURES
            ? ' [DEGRADED]'
    ...
    getHealthReport(): string {
        const h = this.health;
        const successRate = h.totalAttempts > 0
            ? Math.round((h.totalSuccesses / h.totalAttempts) * 100)
            : 0;
        const mode = h.consecutiveFailures >= LearningEngine.MAX_CONSECUTIVE_FAILURES
            ? (this.selfHealer ? '⚠ SELF-HEALING (Anthropic diagnosing)' : '⚠ DEGRADED (heuristic-only)')
            : '✓ Normal';
        ...
        const lines = [
            ...
            h.lastFailure ? `Last failure: ${h.lastFailure} (${h.lastFailureClass})` : '',
