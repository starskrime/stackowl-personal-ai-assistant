...
export class KnowledgeGraphManager {
     ...
     */
    touchDomain(domain: string, source: DomainNode['source'] = 'conversation'): void {
     ...
     */
    recordStudy(domain: string, pelletsCreated: number, relatedTopics: string[]): void {
        const normalized = domain.trim().toLowerCase();
        ...
        existing.depth = Math.min(1.0, existing.depth + DEPTH_GAIN * gainMultiplier);
        ...
        const newRelated = relatedTopics.map(t => t.trim().toLowerCase()).filter(Boolean);
        existing.relatedTopics = [...new Set([...existing.relatedTopics, ...newRelated])];

        this.graph.domains[normalized] = existing;

        // Remove from study queue — we just studied it
        this.graph.studyQueue = this.graph.studyQueue.filter(t => t !== normalized);

        // Add frontier topics to study queue (only if we haven't studied them before)
        for (const related of newRelated) {
            if (!this.graph.domains[related] && !this.graph.studyQueue.includes(related)) {
    ...
    getStats(): { totalDomains: number; avgDepth: number; studyQueueLength: number } {
    ...
    getFullReport(): string {
        ...
        const domains = Object.entries(this.graph.domains)
            .sort(([, a], [, b]) => b.depth - a.depth);
        ...
        if (domains.length > 0) {
            ...
            for (const [name, node] of domains) {
                const bar = '█'.repeat(Math.round(node.depth * 10)) + '░'.repeat(10 - Math.round(node.depth * 10));
                lines.push(`${bar} ${Math.round(node.depth * 100)}%  ${name}  (${node.pelletCount} pellets, ${node.studyCount} sessions)`);
            }
            lines.push('');
        }
