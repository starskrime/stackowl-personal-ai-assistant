...
export class CapabilityLedger {
  ...
  async record(proposal: ToolProposal): Promise<void> {
    ...
    const idx = this.manifest.tools.findIndex(
      (t) => t.toolName === proposal.toolName,
    );
    if (idx >= 0) {
  ...
  async findExisting(userRequest: string): Promise<ToolRecord | undefined> {
    ...
    for (const tool of active) {
      const haystack =
        `${tool.toolName.replace(/_/g, " ")} ${tool.description}`.toLowerCase();
      const matchCount = requestWords.filter((kw) =>
