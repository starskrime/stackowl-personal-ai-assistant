...
export class Logger {
  ...
  llmRequest(model: string, messages: ChatMessage[]): void {
    ...
    for (const msg of messages) {
      ...
      const roleLabel =
        msg.role === "system"
          ? chalk.dim.italic("[SYSTEM]")
          : msg.role === "user"
            ? chalk.green("[USER]")
            : msg.role === "assistant"
              ? chalk.yellow("[ASSISTANT]")
              : msg.role === "tool"
                ? chalk.blue(`[TOOL:${(msg as any).name ?? "?"}]`)
                : chalk.gray(`[${roleStr}]`);

      let body = msg.content || "";
      if (msg.toolCalls && msg.toolCalls.length > 0) {
        const callLines = msg.toolCalls
          .map(
            (t) =>
  ...
  toolCall(name: string, args?: Record<string, unknown>): void {
    ...
    console.log(
      `${this.prefix("info")} ${chalk.bold.yellow("⚙")} ${chalk.yellow(`CALL  ${name}`)}${argsStr}`,
    );
  }
  ...
  toolResult(name: string, result: string, success: boolean): void {
    ...
    fileLog(
      `[TOOL] RESULT ${name} ${success ? "✓" : "✗"} ${result.slice(0, 500)}`,
    );
  ...
  model(selected: string, reason?: string): void {
    ...
    console.log(
      `${this.prefix("info")} ${chalk.dim("model →")} ${chalk.cyan(selected)}${r}`,
    );
  }
  ...
  evolve(msg: string): void {
    ...
    console.log(
      `${this.prefix("info")} ${chalk.bold.magenta("🧬")} ${chalk.magenta(msg)}`,
    );
  }
