...
export class ConstellationMiner {
  ...
  constructor(
    ...
    if (!existsSync(this.constellationDir)) mkdirSync(this.constellationDir, { recursive: true });
  }
      ...
      const resp = await this.provider.chat(
        [{
          ...
          content:
            ...
            (existingTitles.length > 0 ? `Already discovered: ${existingTitles.join(', ')}. Find NEW patterns only.\n\n` : '') +
            `Respond with JSON:\n` +
