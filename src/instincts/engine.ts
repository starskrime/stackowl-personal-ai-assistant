...
export class InstinctEngine {
    ...
    async evaluate(
        ...
        try {
            ...
            const parsed = JSON.parse(jsonStr) as { triggered: boolean; instinctId: string | null };

            if (parsed.triggered && parsed.instinctId) {
                const triggered = availableInstincts.find(i => i.name === parsed.instinctId);
                if (triggered) {
            ...
            log.engine.warn(`[Instinct] Batch evaluation failed: ${err instanceof Error ? err.message : String(err)}`);
            return null;
        }
    }
}
