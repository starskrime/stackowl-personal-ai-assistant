...
- [x] **T1 — Asserted-not-measured success → one AcceptanceAuthority** (ADR-1)
...
- [x] **T2 — Scattered give-up → one RecoveryActuator ladder** (ADR-2)
...
- [x] **T3 — Ask-first reflex → one ReversibilityResolver** (ADR-3)
...
- [x] **T4 — Registered ≠ reachable → a Reachability invariant at boot** (ADR-4)
...
- [x] **T5 — Learning on an untrustworthy signal (positive-only) → verified-gated learning** (ADR-5)
...
- [x] **T6 — Detect-only lifecycle → closed detect→heal→verify loop** (ADR-6)
...
- [x] **T7 — No decision trace → one DecisionLedger the authorities emit to** (ADR-7)
